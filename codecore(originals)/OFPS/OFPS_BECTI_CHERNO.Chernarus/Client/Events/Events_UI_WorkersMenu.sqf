...
switch (_action) do {
	...
	case "onMapButtonDown": {
		...
		if (_button isEqualTo 0 && (uiNamespace getVariable "cti_dialog_ui_workersmenu_sellmode") isEqualTo 1) then {
			...
			_nearest_wip = [_mappos, _structures_wip] call CTI_CO_FNC_GetClosestEntity;
			...
			//--- Deal with a ruins structure
			if (_nearest_wip distance _mappos < 10) then {
				if (isNil {_nearest_wip getVariable "cti_sell"}) then {
					_nearest_wip setVariable ["cti_completion", -2];
					//announce ruins removal
					_coords = mapGridPosition _nearest_wip;
					["structure-sold", ["Ruins", _coords]] remoteExec ["CTI_PVF_CLT_OnMessageReceived", CTI_P_SideJoined];
				};
			} else {
				...
				if (_nearest distance _mappos < 50 && isNull _fobr && isNull _foblarger) then {
					if (isNil {_nearest getVariable "cti_sell"}) then {
						...
						if ((typeOf _nearest) == "Land_Airport_02_controlTower_F") then {
			...
			(CTI_P_SideJoined) call CTI_SE_FNC_UpdateBaseAreas;
