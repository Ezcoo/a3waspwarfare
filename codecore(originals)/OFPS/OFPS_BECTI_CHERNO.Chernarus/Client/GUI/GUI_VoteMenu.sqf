...
while { alive player && dialog } do {
	...
	_vote_time = CTI_P_SideLogic getVariable ["cti_votetime", -1];
	
	if (_vote_time < 0 && !(ctrlShown ((uiNamespace getVariable "cti_dialog_ui_votemenu") displayCtrl 300005))) exitWith {closeDialog 0};
	...
	if !(ctrlShown ((uiNamespace getVariable "cti_dialog_ui_votemenu") displayCtrl 300005)) then {
		{
		...
		_vote = (group player) getVariable "cti_vote";
		_index = (uiNamespace getVariable "cti_dialog_ui_votemenu_groups") find _vote;
		_index = [_index + 1, 0] select (_index < 0);
		_vote_array set [_index, (_vote_array select _index) + 1];
		_player_count = _player_count + 1;
	...
	_voted_commander = if ((_vote_array select _highest_id) <= (_player_count/3) || _highest_id isEqualTo 0) then {"Electing: No Commander"} else {format["Electing: %1", name leader ((uiNamespace getVariable "cti_dialog_ui_votemenu_groups") select _highest_id-1)]};
