...
while { true } do {
	...
	if (time - _last_infocheck > 1 || uiNamespace getVariable "cti_dialog_ui_transferresourcesmenu_forceupdate") then {
		...
		if (!(_income_commander isEqualTo _last_income_commander) || !(_income_players isEqualTo _last_income_players)) then {
			((uiNamespace getVariable "cti_dialog_ui_transferresourcesmenu") displayCtrl 140010) ctrlSetStructuredText (parseText format["<t align='left'>Commander's Town Income:</t> <t align='right' color='%1'>$%2</t>", CTI_P_Coloration_Money, _income_commander]);
			((uiNamespace getVariable "cti_dialog_ui_transferresourcesmenu") displayCtrl 140011) ctrlSetStructuredText (parseText format["<t align='left'>Player's Town Income:</t> <t align='right' color='%1'>$%2</t>", CTI_P_Coloration_Money, _income_players]);
	...
	if (!isNil {uiNamespace getVariable "cti_dialog_ui_transferresourcesmenu_group"}) then {
		...
		if !(_funds isEqualTo _last_grp_funds) then {
			((uiNamespace getVariable "cti_dialog_ui_transferresourcesmenu") displayCtrl 140003) ctrlSetStructuredText (parseText format["<t align='left'>Town Income % for Players:</t> <t align='right' color='%1'>$%2</t>", CTI_P_Coloration_Money, _funds]);
