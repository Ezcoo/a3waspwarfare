								...
								if (count _gear_sub isEqualTo 2) then {
									...
									if (typeName (_gear_sub select 0) isEqualTo "ARRAY") then {
										if (count(_gear_sub select 0) isEqualTo 2) then {
											//--- Attempt to fix wrong OFP templates definitions without altering the source
											_gear_array = [(_gear_sub select 0) select 0];
											
											switch (typeName ((_gear_sub select 0) select 1)) do {
												case "ARRAY": {{_gear_array pushBack _x} forEach ((_gear_sub select 0) select 1)}; //--- Handle setups like laser designators (item + batteries)
												case "STRING": {_gear_array pushBack ((_gear_sub select 0) select 1)}; //--- Handle setups like nvg
											};
											
											{
												...
												if !(_x isEqualTo "") then {
													...
													//--- The item is either a magazine like a Laser battery or item like a pair of binoculars
													if (_x in _side_gear && !(_config_type isEqualTo "nil")) then {
														//--- In case of a Weapon Class, make sure that it's an Item
														if (_config_type isEqualTo "CfgWeapons") then {
															if !(getNumber(configFile >> "CfgWeapons" >> _x >> "type") isEqualTo CTI_TYPE_EQUIPMENT) then {
																_flag_load = false; 
																_err_reason = format["Equipment (Binoc/NVG) [%1] type does not belong to the Equipment type", _x];
															};
														};
													} else {
														_flag_load = false;
														_err_reason = format["Equipment (Binoc/NVG) [%1] is either not a valid CfgWeapons/CfgMagazines class or does not belong to the player's side", _x];
													};
												};
												
												if !(_flag_load) exitWith {}; //--- Do not process the next item if the first one is fubar
											} forEach _gear_array;
										} else {
											_flag_load = false;
