...
//--- LogNewHeadlessClient on Database
if (CTI_DATABASE_TRACKING) then {
	[] remoteExec ["CTI_PVF_SRV_LogNewHeadless", CTI_PV_SERVER];
};
...
//--- The Headless Client receives a database information gathering request
	CTI_PVF_CLT_FPS_DatabaseUpdate = {
	private ["_fps", "_uptime","_aiCount","_aiCountLocal", "_scriptCounts"];
		_fps = diag_fps;
		...
		["headlessFps", [_fps, _uptime, _aiCount, _aiCountLocal] + _scriptCounts] remoteExec ["CTI_PVF_SRV_DatabaseUpdate", CTI_PV_SERVER];
	};
	
	/*CTI_PVF_HC_AddCRAMFireEH = { 
		_unit = _this; 
		_unit addEventHandler ["Fired",{_this spawn FNC_CRAMControl_AttackerFiredEH_HC}]; 
	}; 
	
	CTI_PVF_HC_OnCRAMTargetableFired = {
		_shooterSide = _this select 0;
		_projectile = nearestObject [_this select 1, _this select 2];
		//_projectile = _this select 1;
		if (isNull _projectile) then {
			_projectile = (_this select 1) nearObjects [(_this select 2), 500];
			diag_log ("HC received null targetable. Attempted to recover and found: " + str(_projectile));
		} else {
		...
		
		{
			_side = _x;
			{
				//diag_log ("potential attacker firing targetable round recieved " + str(_projectile) + "added to " + str(_side));
				_targets = missionNamespace getVariable [format ["CRAM_Targets_%1", _side], []];
				_targets pushBackUnique _x;
				missionNamespace setVariable [format ["CRAM_Targets_%1", _side], _targets];
			} forEach _projectile;
	...
	CTI_PVF_HC_OnDefenseDelegationReceived = {
		...
		//--- Create the unit
		if (_static call CTI_CO_FNC_IsVehicleUAV) then {
            createVehicleCrew _static;
            _ai =  gunner _static;
            _logic = (_side) call CTI_CO_FNC_GetSideLogic;
            _defense_team = _logic getVariable "cti_defensive_team";

            [_static] joinSilent _defense_team;

			//--- Give order to Patrol
			if(_static isKindOf "B_UAV_01_F" || _static isKindOf "O_UAV_01_F") then {
				//set leader to help with coordination 
				(group (driver _static)) selectLeader  (driver _static);
				//move it
				_static engineOn true;
				_oldpos = position _static;
				_newlevel = [_oldpos select 0, _oldpos select 1, 600];
				(driver _static) move _newlevel;
				_wp = group _static addWaypoint [_newlevel, 0];
	...
	CTI_PVF_HC_OnTownDelegationReceived = {
		...
		if (count _groups > 0) then {		
			...
			if (isNil {_town getVariable _hc_tvar}) then {_town setVariable [_hc_tvar, [], true]};
	...
	CTI_PVF_HC_OnTownDelegationRemoval = {
		...
		if !(isNil {_town getVariable _hc_tvar}) then {
			...
			_town setVariable [_hc_tvar, nil, true];
		};
		...
		_town setVariable [_hc_tvar, [], true];
	...
	CTI_PVF_HC_UpdateTownGroups = {
		...
		_town setVariable [_hc_tvar, (_town getVariable [_hc_tvar, []]) + _groups, true];
...
// VEHICLE TRACKING LOOP (ADD YOUR LOOPY STUFF TO THIS)
//execVM "Client\Functions\Client_Headless_VehicleLoop.sqf";