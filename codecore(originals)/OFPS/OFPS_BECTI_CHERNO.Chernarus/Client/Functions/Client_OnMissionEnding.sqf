...
// Check for econ win
if (_winnerside call CTI_CO_FNC_GetSideTownCount >= round (CTI_TOWN_WIN_MODE / 100 * count CTI_Towns)) then { 
	_winnertext = format ["%1 has secured the region!", _winnername];
	_losertext = format ["%1 failed to hold their ground!", _losername];
} else {
	_winnertext = format ["%1 has dominated the battlefield!", _winnername];
	_losertext = format ["%1 has lost all factories and their MHQ!", _losername];
};
