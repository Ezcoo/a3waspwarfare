...
CTI_UI_Gear_DisplayShoppingItems = {
	...
	_list = switch (_tab) do {
		...
		default {profileNamespace getVariable [format["CTI_PERSISTENT_GEAR_TEMPLATEV6_%1", CTI_P_SideJoined], []]};
	};
	_upgrade_gear = -1;
	//--- Check Source
	...
	if (isPlayer _target) then {
		...
		CTI_Base_GearInRange_AI = [false, true] select !(isNull ([CTI_BARRACKS, _target, _structures, CTI_BASE_GEAR_RANGE] call CTI_CO_FNC_GetClosestStructure));
		CTI_Base_GearInRange_Mobile_AI = [false, true] select (count([_target, CTI_SPECIAL_AMMOTRUCK, CTI_SERVICE_AMMO_TRUCK_RANGE] call CTI_CO_FNC_GetNearestSpecialVehicles) > 0);
		CTI_Base_GearInRange_AmmoPod_AI =  [false, true] select (count([_target, CTI_SPECIAL_GEAR, CTI_SERVICE_GEAR_RANGE] call CTI_CO_FNC_GetNearestSpecialVehicles) > 0);

		// Depot check for AI
		CTI_Base_GearInRange_Depot_AI = false;
		_depot = [_target, CTI_P_SideID] call CTI_CO_FNC_GetClosestDepot;
		if !(isNull _depot) then {
			...
			CTI_Base_GearInRange_Depot_AI = _available;
		} else {
			CTI_Base_GearInRange_Depot_AI = false;
		};
		...
		if (CTI_BASE_FOB_MAX > 0) then {
			_nearest = [_target, CTI_P_SideLogic getVariable ["cti_fobs",[]]] call CTI_CO_FNC_GetClosestEntity;
			if (_nearest distance _target <= CTI_BASE_GEAR_FOB_RANGE) then {_ai_in_range = true};
		};
		CTI_Base_GearInRange_FOB_AI = _ai_in_range;

		//--- Large FOB Check
		_ai_in_range_large = false;
		if (CTI_BASE_LARGE_FOB_MAX > 0) then {
			_nearest = [_target, CTI_P_SideLogic getVariable ["cti_large_fobs",[]]] call CTI_CO_FNC_GetClosestEntity;
			if (_nearest distance _target <= CTI_BASE_GEAR_LARGE_FOB_RANGE) then {_ai_in_range_large = true};
		};
		CTI_Base_GearInRange_LARGE_FOB_AI = _ai_in_range_large;

		/*if (CTI_Base_GearInRange_Depot_AI && !(CTI_Base_GearInRange_AI || CTI_Base_GearInRange_Mobile_AI || CTI_Base_GearInRange_AmmoPod_AI || CTI_Base_GearInRange_FOB_AI || CTI_Base_GearInRange_LARGE_FOB_AI)) then {
			_upgrade_gear = _upgrades select CTI_UPGRADE_TOWNS;
		};*/

		if (!(CTI_Base_GearInRange_AI || CTI_Base_GearInRange_Mobile_AI || CTI_Base_GearInRange_AmmoPod_AI || CTI_Base_GearInRange_FOB_AI || CTI_Base_GearInRange_LARGE_FOB_AI || CTI_Base_GearInRange_Depot_AI)) then { //checks if players is not near a structure/special unit that will allow purchasing of gear. If player isn't near a structure that allows purchasing of gear then player will have an empty equipment list.
			_upgrade_gear = -1;
			_allowPurchase = false;
		};
		ctrlEnable [70403, _allowPurchase];
		ctrlEnable [70030, _allowPurchase];
		ctrlEnable [70029, _allowPurchase];
	};
	...
	if (_tab != CTI_GEAR_TAB_TEMPLATES) then {
...

		{
			...
			if !(isNil "_get") then {
				//systemChat format ["Filter %1", _get];
				_gear_avail = (_x) call CTI_UI_Gear_CheckGearAvailability;
				//--- Add filters
				if (_gear_avail) then {
					if (!((_get select CTI_GEAR_FILTERUI) isEqualTo "")) then {
						//_filters pushBack (_get select CTI_GEAR_FILTERUI);
						{
							if !(_x in _filters) then {
								_filters pushBack (_x);
							};
						} forEach (_get select CTI_GEAR_FILTERUI);
					};
				};
			};
		} forEach _list;
	...
	if !(_tab isEqualTo CTI_GEAR_TAB_TEMPLATES) then { //--- Generic items
		{
			...
			if !(isNil "_get") then {
				...
				_gearfilters = [];
				{
					_gearfilters pushBack (_x);
				} forEach (_get select CTI_GEAR_FILTERUI);
				if (_filter_use isEqualTo "all" || _filter_use in _gearfilters) then {
					if (_gear_avail) then { //--- Check Availability
						_row = lnbAddRow [70108, [ format ["$%1", (_get select CTI_GEAR_PROPERTIES) select 1], getText(configFile >> _get select CTI_GEAR_CONFIG >> _x >> 'displayName'), format["%1", (_get select CTI_GEAR_PROPERTIES) select 0], format["%1", (_get select CTI_GEAR_PROPERTIES) select 5]]];
						lnbSetPicture [70108, [_row, 0], getText(configFile >> _get select CTI_GEAR_CONFIG >> _x >> 'picture')];
		...
		{
			//Check all items in template for availability
			...
			if (_template_avail) then {
				_row = lnbAddRow [70108, [_x select 0, format ["$%1", _x select 2]]];
				if !((_x select 1) isEqualTo "") then {lnbSetPicture [70108, [_row, 0], _x select 1]};
...
CTI_UI_Gear_CheckGearAvailability = {
	private ["_item"];
	_item = _this;
	_gear_available = false;

	//--- Check Upgrades
	_upgrades = (CTI_P_SideJoined) call CTI_CO_FNC_GetSideUpgrades;
	_upgrade_gear = _upgrades select CTI_UPGRADE_GEAR;
	_upgrade_logistics = _upgrades select CTI_UPGRADE_TOWNS;
	//--- Check Source
	_target = uiNamespace getVariable "cti_dialog_ui_gear_target";
	_source =  [_target, CTI_P_SideJoined] call CTI_CO_FNC_GetClosestGearSource;
	
	//--- Start Check
	_get = missionNamespace getVariable format["cti_%1", _item];

	if !(isNil "_get") then {
		//systemChat format ["Item %1", _get];

		if ((_get select CTI_GEAR_PROPERTIES) select 4) then { //--- Add if item enabled
			_gear_source = "";
			_gear_type = "";
			_gear_lvl = 0;
			...
			//check gear upgrade
			{
				...
				switch (typeName _x) do {
					case "STRING": { 
						_gear_source_temp = _x;
						_gear_type_temp = "gear";
						_gear_lvl_temp = ((_get select CTI_GEAR_PROPERTIES) select 0);
						if (_x isEqualTo "All") then {
							_gear_source_temp = "All";
						};
						if (_x isEqualTo "External") then {
							_gear_source_temp = "External";
						};
					};
					case "ARRAY": { 
						//check to source
						_count = count _x;
						_gear_source_temp = _x select 0;
						_gear_type_temp = _x select 1;
						if (_gear_source_temp isEqualTo "All") then {
							_gear_source_temp = "All";
						};
						if (_gear_source_temp isEqualTo "External") then {
							_gear_source_temp = "External";
						};
						if (_count > 2) then {
							_gear_lvl_temp = _x select 2;
						} else {
							_gear_lvl_temp = ((_get select CTI_GEAR_PROPERTIES) select 0);
						};
					};
				};
				if (_gear_source_temp isEqualTo _source) then {
					_gear_source = _gear_source_temp;
					_gear_type = _gear_type_temp;
					_gear_lvl = _gear_lvl_temp;						
				};
				if (_gear_source_temp isEqualTo "All") then {
					_gear_source = _gear_source_temp;
					_gear_type = _gear_type_temp;
				...
				if (_gear_source_temp isEqualTo "External" && !(_source isEqualTo CTI_BARRACKS)) then {
					_gear_source = _gear_source_temp;
					_gear_type = _gear_type_temp;
					_gear_lvl = _gear_lvl_temp;						
				};

			} foreach ((_get select CTI_GEAR_PROPERTIES) select 3);
			
			switch (_gear_type) do {
				case "gear": {
					_gear_available = _gear_lvl <= _upgrade_gear;
				};
				case "logistics": {
					_gear_available = _gear_lvl <= _upgrade_logistics;
				};
				default {_gear_available = false};
			};
			//systemChat format ["Available: %1 | %2 | %3 | %4 | %5", _source, _gear_available, _gear_source, _gear_type, _gear_lvl];
		};
	};
...
CTI_UI_Gear_ReplaceContainer = {
	...
	if !(_item isEqualTo "") then {
		...
		if !(_current isEqualTo _item) then {
			...
			_contentsmass = _contents call CTI_UI_Gear_GetItemsMass;
			_containercapacity = _item call CTI_UI_Gear_GetContainerMassCapacity;

			["Container", "", _item, [_container, _contents]] call CTI_UI_Gear_UpdateMass;
...
CTI_UI_Gear_AddItem = {
			...
			case "Equipment": { //--- Binoc... NVG...
				...
				if (_index isEqualTo 1) then {
					...
					if !(_current isEqualTo _item) then { //--- Replace
						((uiNamespace getVariable "cti_dialog_ui_gear") displayCtrl 70007) ctrlSetText getText(configFile >> 'CfgWeapons' >> _item >> 'picture');
						((uiNamespace getVariable "cti_dialog_ui_gear") displayCtrl 70007) ctrlSetTooltip getText(configFile >> 'CfgWeapons' >> _item >> 'displayName');

						(((_gear select 3) select 0) select 1) set [0, _item];
						_updated = true;
					};
				} else {
					if !(_current isEqualTo _item) then { //--- Replace
						((uiNamespace getVariable "cti_dialog_ui_gear") displayCtrl 70006) ctrlSetText getText(configFile >> 'CfgWeapons' >> _item >> 'picture');
						((uiNamespace getVariable "cti_dialog_ui_gear") displayCtrl 70006) ctrlSetTooltip getText(configFile >> 'CfgWeapons' >> _item >> 'displayName');

						((_gear select 3) select 0) set [0, _item];
						_updated = true;
					};
				};
			};
	...
	if (_updated) then {_this call CTI_UI_Gear_CheckNVGCompatible;};
	_updated
};
...
CTI_UI_Gear_TryEquipAccessory = {
	...
	_weaponIndex = if (count _this > 2) then {_this select 2} else {-1};
	...
	_match = false;
	{
		if (_weaponIndex == _forEachIndex || _weaponIndex == -1) then {
			//systemChat format ["Replacing %1 accessory for weapon %2, specific: %3", _accessory, _forEachIndex, _weaponIndex];
			if !((_x select 0) isEqualTo "") then {
...
CTI_UI_Gear_GetTotalMass = {
	...
	//--- Special and items
	_nvg_mass = [(((_gear select 3) select 0) select 0)] call CTI_UI_Gear_GetItemsMass;
	_bino_w_mag_mass = (((_gear select 3) select 0) select 1);
	if(_bino_w_mag_mass isEqualType "") then {_bino_w_mag_mass = [_bino_w_mag_mass];};
	_bino_w_mag_mass = _bino_w_mag_mass call CTI_UI_Gear_GetItemsMass;
...
// Checks and removes NVGs if the headgear is incompatible with them.
CTI_UI_Gear_CheckNVGCompatible = {
	private _item = _this;
	_config_type = (_item) call CTI_UI_Gear_GetItemBaseConfig;
	if (count getArray(configFile >> _config_type >> _item >> 'subItems') > 0) then {
		_slot = [0,0];
		_idc = 70006;
...
CTI_UI_Gear_UpdateLinkedItems = {
	...
	if (_config_type isEqualTo "CfgWeapons") then {
...

		{
			...
			if (!(isNil "_get") && _gear_avail) then {
				_row = lnbAddRow [70601, [format ["$%1", (_get select 0) select 1],getText(configFile >> _get select 2 >> _x >> 'displayName'),format ["%1", (_get select 0) select 0],format ["%1", (_get select 0) select 5]]];
				lnbSetPicture [70601, [_row, 0], getText(configFile >> _get select 2 >> _x >> 'picture')];
		...
		case "Weapon": {
			switch (_type select 1) do {
				...
				case "Launcher" : {
					...
					if(!_reloadable) then {
						_magazineclass = getArray(configFile >> "CfgWeapons" >> _classname >> "magazines") select 0;
				...
				default {
					...
					private _modes = [];
					{
						...
						if(getNumber(_mode >> "showToPlayer") == 1) then {
							...
							if (getNumber(_mode >> "autoFire") == 1) then {
								...
								if (getNumber(_mode >> "autoFire") == 0) then {
									if(getNumber(_mode >> "burst") > 1) then {
										_fmstr = format["%1rnd",getNumber(_mode >> "burst")];
									} else {
...
CTI_UI_Gear_EquipTemplate = {
	...
	private ["_config_base", "_gear", "_load", "_startidc", "_startidc_current_mag", "_use", "_use_id"];
	_gear_old = +(uiNamespace getVariable "cti_dialog_ui_gear_target_gear");
	_gear = +(((profileNamespace getVariable format["CTI_PERSISTENT_GEAR_TEMPLATEV6_%1", CTI_P_SideJoined]) select _index) select 3);
	_gear_flat = (_gear) call CTI_CO_FNC_ConvertGearToFlat;
	_gear_current = uiNamespace getVariable "cti_dialog_ui_gear_target_gear";
	...
	{	
		...
		{	
			...
			if (_forEachindex == 0) then {
				if !((_x) isEqualTo "") then {
					[_x, _weaponIndex, true] call CTI_UI_Gear_ReplaceWeapon;
				};
				_startidc = _startidc + 1;
			} else {
				// Replace Attachments if needed
				if (_forEachindex == 1) then {
					{
						//systemChat str _x;
						if !((_x) isEqualTo "") then {
							//(_x) call CTI_UI_Gear_AddItem;
							_get = missionNamespace getVariable format["cti_%1", _x];
							if !(isNil '_get') then {
								_sub_type = if (typeName (_get select 1) isEqualTo "STRING") then {_get select 1} else {(_get select 1) select 1};
								_updated = [_x, _sub_type, _weaponIndex] call CTI_UI_Gear_TryEquipAccessory;
							};
						};
						_startidc = _startidc + 1;
					} forEach _x;
				} else {
					// Replace Magazines in weapon
					if (_forEachindex == 2) then {
						[_x select 0, _weaponIndex, 70901+_weaponIndex] call CTI_UI_Gear_ChangeCurrentMagazine;
						_gear_current select 0 select _weaponIndex set [2, _gear select 0 select _weaponIndex select 2];
					};
				};
			};
		} forEach _x;
	...
	// Replace the containers
	{
		...
		["", _containerIndex] call CTI_UI_Gear_ReplaceContainer;
		[_gear_old select 1 select _containerIndex select 0, _containerIndex] call CTI_UI_Gear_ReplaceContainer; // Empty container
		if !(_x select 0 isEqualTo "") then {
			[_x select 0, _containerIndex] call CTI_UI_Gear_ReplaceContainer;
		};
	} forEach (_gear select 1);
	
	_noFit = [];
	
	// Try to add things that did not fit into the template again
	{
		_containerIndex = _forEachIndex;
		{
			_updated = [_x, _containerIndex, false] call CTI_UI_Gear_TryContainerAddItem;
			if !(_updated) then {
				_noFit pushBack _x;
			};
		} forEach (_x select 1);
	...
	// Try to allocate things that did not fit.
	{
		[_x, 0] call CTI_UI_Gear_TryContainerAddItem;
	} forEach (_noFit);
	
	_magazines_ignore = [];
	{	
		_magazines = ((getArray(configFile >> 'CfgWeapons' >> _x select 0 >> 'magazines')) call CTI_CO_FNC_ArrayToLower);
		{
			// Only keep the old magazines if there are no replacements in the new template
			if (_x in _gear_flat) then {
				_magazines_ignore pushBack _x;
			} forEach _magazines;
		};
	} forEach (_gear_old select 0);
	
	// Magazines to keep if there are none in the new template.
	_magazines_keep = [];
	{
		_magazines_keep = ((getArray(configFile >> 'CfgWeapons' >> _x select 0 >> 'magazines')) call CTI_CO_FNC_ArrayToLower);
		{
			
		};
	} forEach (_magazines_ignore);
	
	_ignoredItems = _gear_flat + _magazines_ignore;
	
	// Now we do the same with items from the old gear.
	_noFit = [];
	{
		_containerIndex = _forEachIndex;
		{
			// Don't carry over items from old gear if they are already taken care of by our template
			if !(_x in _ignoredItems) then {
				_updated = [_x, _containerIndex, false] call CTI_UI_Gear_TryContainerAddItem;
				if !(_updated) then {
					_noFit pushBack _x;
				};
	...
	// Helmet/Facewear
	{
		if !(_x isEqualTo "") then {
			(_x) call CTI_UI_Gear_AddItem;
		} else { // Readd old one so new NVGs get removed if necessary due to the helmet
			if !((_gear_old select 2 select _forEachIndex) isEqualTo "") then {
				(_gear_old select 2 select _forEachIndex) call CTI_UI_Gear_AddItem;
			};
		};
	} forEach (_gear select 2);
	
	// Other Equipment/Gadgets
	{
		if !(_x isEqualTo "") then {
			(_x) call CTI_UI_Gear_AddItem;
		};
...
CTI_UI_Gear_LoadAvailableUnits = {
	...
	
	{
		_nearest = [CTI_BARRACKS, _x, _structures, CTI_BASE_GEAR_RANGE] call CTI_CO_FNC_GetClosestStructure;
		_ammo_trucks = [_x, CTI_SPECIAL_AMMOTRUCK, CTI_BASE_GEAR_RANGE/4] call CTI_CO_FNC_GetNearestSpecialVehicles;
		_depot_in_range = false;
		_fob_in_range = false;
		_large_fob_in_range = false;
		_depot = [_x, CTI_P_SideID] call CTI_CO_FNC_GetClosestDepot;
		if !(isNull _depot) then {
			_available = false;
			switch (CTI_TOWNS_DEPOT_ACCESS_MODE) do {
				case 1: {if ([_depot getVariable "cti_depot", CTI_P_SideID] call CTI_CO_FNC_HasAllCamps) then {_available = true}}; //--- Check if the camps belong to the player's side
		...
		if (!isNull _nearest || _x isEqualTo player || count _ammo_trucks > 0 || _fob_in_range || _large_fob_in_range || _depot_in_range) then {//todo add fob
			_list pushBack _x;
			if (isPlayer _x) then {
				((uiNamespace getVariable "cti_dialog_ui_gear") displayCtrl 70201) lbAdd Format["[%1] %2", _x call CTI_CL_FNC_GetAIDigit, name _x, getText(configFile >> "CfgVehicles" >> typeOf _x >> "displayName")];
			} else {
				_sp = (name _x) find " ";
				_name = name _x;
				if!(_sp isEqualTo -1) then {
					_name = _name select [(_sp + 1)];
				};
				((uiNamespace getVariable "cti_dialog_ui_gear") displayCtrl 70201) lbAdd Format["[%1] %2 - %3", _x call CTI_CL_FNC_GetAIDigit, _name, getText(configFile >> "CfgVehicles" >> typeOf _x >> "displayName")];
			};
		};
		if (!(vehicle _x isEqualTo _x) && !(vehicle _x in _vehicles)) then { //--- Vehicle check
			if (getNumber(configFile >> "CfgVehicles" >> typeOf vehicle _x >> "maximumLoad") > 0) then {
				if (!isNull _nearest || count _ammo_trucks > 0 || _fob_in_range || _large_fob_in_range || _depot_in_range) then {
	...
	{
		_crew = fullCrew (vehicle _x); _driver = []; _commander = []; _gunner = []; _turret = []; _cargo = []; _other = [];
		{
			_digit = (_x call CTI_CL_FNC_GetAIDigit);
			switch (_x select 1) do {
				case "driver":{_driver pushBack _digit};
				case "commander":{_commander pushBack _digit};
				case "gunner":{_gunner pushBack _digit};
				case "turret":{_turret pushBack _digit};
				case "cargo":{_cargo pushBack _digit};
				default {
					_other pushBack _digit;
				};
			};
		} forEach _crew;
		_crewDigits = [];
		{
			if(count _x > 0) then {
				_crewDigits append [(_x joinString ", ")];
			}
		} forEach [_driver,_commander, _gunner, _turret, _cargo, _other];
		_crewDigits = _crewDigits joinString ", ";
		_list pushBack vehicle _x;
		((uiNamespace getVariable "cti_dialog_ui_gear") displayCtrl 70201) lbAdd Format["%1 [%2]", getText(configFile >> "CfgVehicles" >> typeOf (vehicle _x) >> "displayName"), _crewDigits];
	} forEach _vehicles;
	//diag_log [_list, _vehicles];
	{
		_nearest = [CTI_BARRACKS, _x, _structures, CTI_BASE_GEAR_RANGE] call CTI_CO_FNC_GetClosestStructure;
		_ammo_trucks = [_x, CTI_SPECIAL_AMMOTRUCK, CTI_BASE_GEAR_RANGE/4] call CTI_CO_FNC_GetNearestSpecialVehicles;
		_fob_in_range = false;
		...
		if (!(_x in _vehicles) && ({side _x isEqualTo side player} count (crew _x)) > 0) then { //--- Vehicle check
...
CTI_UI_Gear_RenameProfileTemplate = {
