CTI_UI_Respawn_GetAvailableLocations = {
	private ["_fobs","_large_fobs", "_hq", "_ignore_mobile_crew", "_list", "_mobile", "_structures","_up","_respawnrangefob","_respawnrangelargefob", "_respawnrangedepot"];
	...
	if ((missionNamespace getVariable "CTI_RESPAWN_CAMPS") > 0) then {
		...
		// Add depots
		_upl=if (!( count ((CTI_P_SideJoined) call CTI_CO_FNC_GetSideUpgrades) isEqualTo 0)) then {((CTI_P_SideJoined) call CTI_CO_FNC_GetSideUpgrades) select CTI_UPGRADE_REST} else {0};
		_respawnrangedepot=CTI_RESPAWN_CAMPS_RANGE_ENHANCED+500*_upl;
		{
			if ([_x getVariable "cti_depot", CTI_P_SideID] call CTI_CO_FNC_HasAllCamps && (_x getVariable "cti_depot") getVariable "cti_town_sideID" isEqualTo (CTI_P_SideID)) then {
				//if (_x distance CTI_DeathPosition <= _respawnrangedepot) then {
					_cti_entities = _x nearEntities[["Man","Car","Motorcycle","Tank","Air","Ship"], CTI_TOWNS_CAPTURE_RANGE];
...
CTI_UI_Respawn_GetMobileRespawn = {
...

	{
    	...
    	if (typeName _special_type isEqualTo "ARRAY") then {
			...
			if(CTI_SPECIAL_MEDICALVEHICLE in _special_type) then {
				 _special_type = CTI_SPECIAL_MEDICALVEHICLE;
			}
    	};
...
CTI_UI_Respawn_GetRespawnLabel = {
	...
	switch (true) do {
		...
		case (!isNil {_location getVariable "cti_depot"}) : {
			_town = _location getVariable "cti_depot";
			_value = format["Depot (%1)", _town getVariable "cti_town_name"];
		};
		case (_location isKindOf "AllVehicles"): {
			_descr = "";
			_pname = "";
			_originalowner = _location getVariable "cti_originalowner";
			if !(isNil "_originalowner") then {
				// Find the vehicle original owner's player object
				_owner_player = {if ((getPlayerUID _x) isEqualTo _originalowner) exitWith {_x};} forEach allPlayers;
				_pname = name _owner_player;
				if (_originalowner isEqualTo (getPlayerUID player)) then {
					_descr = "%1 (%2)";
				} else {
					_descr = "%1 (%2) - $%3";
				};
			...
			if (isNil "_pname") then {_pname = "Commander";};
			_value = format[_descr, getText(configFile >> "CfgVehicles" >> typeOf _location >> "displayName"), _pname, CTI_RESPAWN_MOBILE_PRICE]; };
	};
...
CTI_UI_Respawn_OnRespawnReady = {
	...
	if !(_respawn_ai) then { //--- Stock respawn
		...
		if (_where isKindOf "AllVehicles") then { //--- Vehicle respawn
			...
			//--- When spawning at another player's vehicle, pay your share to the player who bought it
			_originalowner = _where getVariable "cti_originalowner";
			if (!(isNil "_originalowner") && {!(_originalowner isEqualTo (getPlayerUID player))}) then {
				(-CTI_RESPAWN_MOBILE_PRICE) call CTI_CL_FNC_ChangePlayerFunds;
				// Find the vehicle original owner's player object
				_owner_player = {if ((getPlayerUID _x) isEqualTo _originalowner) exitWith {_x};} forEach allPlayers;
				// If the player is still here, send the money
				if (!isNil "_owner_player") then {
					[group _owner_player, CTI_RESPAWN_MOBILE_REWARD] call CTI_CO_FNC_ChangeFunds;
					["info", [format["Player spawned on your vehicle <br/><t color='#85bb65'>+$%1</t>",CTI_RESPAWN_MOBILE_REWARD]]] remoteExec ["CTI_CL_FNC_NotificationHandle", _owner_player];
					//["Player spawned on your vehicle", "other", CTI_RESPAWN_MOBILE_REWARD] remoteExec ["CTI_DisplayReward_Display", _owner_player];
				};
			}
		};
		...
		if !(_spawn_in) then { //--- If the unit did not respawn in a vehicle, we place it near
			...
			switch (CTI_RESPAWN_BASE_MODE) do {
				case 1: { //--- The player should use the given structure position if we're dealing with a base structure
					if (_where in ((CTI_P_SideJoined call CTI_CO_FNC_GetSideStructures) + CTI_Depots)) then {
						_bposlist = (missionNamespace getVariable [format["CTI_%1_%2", CTI_P_SideJoined, typeOf _where], missionNamespace getVariable format["CTI_%1_%2", CTI_P_SideJoined, "Land_Research_house_V1_F"]]) select CTI_STRUCTURE_RESPAWNBPOS;
						if (_where in CTI_Depots) then { // Weird thing with depots
							_bposlist = -1;
						}; 
						_bpos = [0,0,0];
						...
						_playernearpos = !((_bpos nearEntities ["Man", 2]) isEqualTo []);
						if (!(isNil "_bpos") && !(_bpos isEqualTo [0,0,0]) && !_playernearpos) then {
							_spawn_at = _bpos;
						};
					};
				};
			...
			if(surfaceIsWater _spawn_at) then {
				player setPosASL _spawn_at;
			} else {
	...
	// Set vars for use for transport rewards
	player setVariable ["cti_last_pos", position player, true];
	player setVariable ["cti_last_town", objNull, true];
	// Server need the EH readded after player dies
	//player remoteExec ["CTI_PVF_SRV_AddCRAMFireEH", CTI_PV_SERVER];
