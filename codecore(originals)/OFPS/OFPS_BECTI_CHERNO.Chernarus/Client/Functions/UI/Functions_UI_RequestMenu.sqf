	...
	if (_answer isEqualTo "accept") then { //--- The request was granted by the com
		switch (_request) do {
			...
			case CTI_REQUEST_FOB_DISMANTLE: {
				...
				[] spawn {
					scriptName "FOB Dismantle Permission Timeout";
					_timeEnd = time + CTI_REQUESTS_PERMISSION_TIMEOUT;
					waitUntil {CTI_P_TeamsRequests_FOB_Dismantle != 1 || time > _timeEnd};
					if (time > _timeEnd) then {
						hint parseText "<t size='1.3' color='#2394ef'>Information</t><br /><br />Your FOB dismantling permission has expired";
