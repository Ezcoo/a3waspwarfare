...
if (_side_new isEqualTo CTI_P_SideJoined) then { //--- The player's side has captured it
	...
	if (_in_range) then { //--- The player or one of his unit was in range of the camp.
		...
		//["capture",["camp",_town,_value,_score]] call CTI_CL_FNC_NotificationHandle;
		[format ["Captured a camp", _town], "capture", _value] call CTI_DisplayReward_Display;
		if ([_town, CTI_P_SideJoined] call CTI_CO_FNC_HasAllCamps) then {
			["hint-town-camp-capture"] call CTI_CL_FNC_DisplayMessage;
		};
	};
