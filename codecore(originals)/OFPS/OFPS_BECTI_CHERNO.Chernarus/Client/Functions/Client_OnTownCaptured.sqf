...
if (_side_new isEqualTo CTI_P_SideJoined) then { //--- The player's side has captured it
	...
	CTI_P_ChatID commandChat format[localize "STR_OFPS_CHAT_Town_Captured", _town getVariable "cti_town_name"];
	...
	if (_in_range) then { //--- The player or one of his unit was in range of the town.
		...
		if (_value > 0) then {(_value) call CTI_CL_FNC_ChangePlayerFunds}; //--- Award some money
		//["capture",["town",_town,_value,_score]] call CTI_CL_FNC_NotificationHandle;
		[format ["Captured %1", _town], "capture", _value] call CTI_DisplayReward_Display;
