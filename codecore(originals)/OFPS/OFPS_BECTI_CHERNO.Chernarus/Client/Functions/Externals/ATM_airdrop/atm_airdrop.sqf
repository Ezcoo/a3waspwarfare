...
if ((typeOf (vehicle _target) in CTI_HALO_BLACKLIST) || (vehicle _target isEqualTo ((CTI_P_SideJoined) call CTI_CO_FNC_GetSideHQ))) exitWith {hintsilent "HALO May not be used with the current vehicle"; sleep 1 ; hintsilent ""};
	
_funds = (group player) call CTI_CO_FNC_GetFunds;
_cost = CTI_HALO_COST;
if (_target != vehicle _target) then {
	_var = missionNamespace getVariable typeOf(vehicle _target);
...
if (_funds < _cost) exitWith {hintsilent "Not enough funds"; sleep 1 ; hintsilent ""};
...
//HINT
hint parseText format["<t size='2' color='#2394ef'>HALO JUMP</t><br /><br />Select your destination ! <br /> <t color='#ccffaf'> You can only jump within owned towns or near FOBs (with upgrade).</t><br /> <t color='#0cff00'> $%1 per Jump.</t>", _cost];
...
onMapSingleClick '
	...
	if (_upgrade_halo > 1) then {
		if (((ATM_Jump_clickpos distance  _ct < CTI_MARKERS_TOWN_AREA_RANGE) && [_ct, CTI_P_SideJoined] call CTI_CO_FNC_HasAllCamps) || (ATM_Jump_clickpos distance  _large_fob < CTI_MARKERS_TOWN_AREA_RANGE) || (ATM_Jump_clickpos distance  _base < CTI_BASE_AREA_RANGE)) then {
			ATM_Jump_mapclick = true; onMapSingleClick ""; true;
		};
	} else {
		if (((ATM_Jump_clickpos distance  _ct < CTI_MARKERS_TOWN_AREA_RANGE) && [_ct, CTI_P_SideJoined] call CTI_CO_FNC_HasAllCamps) || (ATM_Jump_clickpos distance  _base < CTI_BASE_AREA_RANGE)) then {
			ATM_Jump_mapclick = true; onMapSingleClick ""; true;
		};
	};
';
...
if (_target isEqualTo vehicle _target) then {
	...
	_paradrop_handler = [vehicle _target, [_x,_y,_z+Altitude]] Spawn CTI_CO_FNC_ParadropVehicle;
	
	waitUntil {scriptDone _paradrop_handler};
