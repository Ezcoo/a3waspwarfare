...
switch (_message_var) do {
	...
	case "award-bounty-defense": {
		player groupChat format ["$%1 awarded for the neutralization of %2 defense", _parameters select 0, _parameters select 1];
	};
	case "build-by": {
	...
	case "building-decay": {hint parseText format ["<t size='1.3' color='#2394ef'>Information</t><br /><br />Supply depleted. Base decay has begun and at <t color='#ff0000'>%1%2</t>", _parameters, "%"]};
	case "building-decay-repair": {hint parseText "<t size='1.3' color='#2394ef'>Information</t><br /><br />This structure cannot be repaired since our side no longer has any Supply!."};
...
//		hint parseText format ["<t align='center'>Enemy Structure Damaged</t><br />Current Enemy Structure Health: %1%2<br /><t align='center'>Enemy base health upgrade level: %3</t>", _parameters select 0, "%", _parameters select 2];
	...
	case "commander-vote-end": {
		if (isNull _parameters) then {
			CTI_P_ChatID commandChat localize "STR_OFPS_CHAT_No_Commander";
		} else {
			if !(_parameters isEqualTo group player) then {
				CTI_P_ChatID commandChat format[localize "STR_OFPS_CHAT_VoteForNewCommanderEnd", name leader _parameters];
				["hint-subordinate"] spawn CTI_CL_FNC_DisplayMessage;
			} else {
				CTI_P_ChatID commandChat localize "STR_OFPS_CHAT_PlayerCommander";
				["hint-commander"] spawn CTI_CL_FNC_DisplayMessage;
			};
		};
	};
	...
	case "defense-destroyed": {
		_var = missionNamespace getVariable (_parameters select 0);
		CTI_P_ChatID commandChat format ["A %1 has been destroyed at %2!", (_var select CTI_DEFENSE_LABEL), mapGridPosition (_parameters select 1)];
	};
	...
	case "fob-dismantled": {
		CTI_P_ChatID commandChat format ["A %1 has been dismantled at %2 by %3", (_parameters select 0), mapGridPosition (_parameters select 1), (_parameters select 2)];
	};
	case "hq-destroyed": {
		CTI_P_ChatID commandChat "The HQ has been destroyed!";
		["hint-hq-destroyed"] spawn CTI_CL_FNC_DisplayMessage;
...
//		playsound "Acts_carFixingWheel"; if sound is recommended, find a better sound
	...
	case "base-area-removed": {
		CTI_P_ChatID commandChat "A Base Area has been removed";
	};
...
//		CTI_P_ChatID sideChat format ["A friendly %1 was killed by the %2 group! %2 was fined $%3", _var select CTI_UNIT_LABEL, (_parameters select 1) getVariable ["cti_alias",CTI_PLAYER_DEFAULT_ALIAS], _parameters select 2];
	...
	case "repair-by": {
		_var = missionNamespace getVariable format ["CTI_%1_%2", CTI_P_SideJoined, _parameters select 1];
		_sv_consumed = "";
		//--- If SV consumption with Repair truck is enabled, notify the player about the consumed SV
		switch (CTI_VEHICLES_REPAIRTRUCK_BASE_REPAIR_USE_SV) do {
			case 1: { //--- SV Cost is based on the repair value (CTI_VEHICLES_REPAIRTRUCK_BASE_REPAIR * 100)
				if ((_parameters select 3) > 0) then {_sv_consumed = format [" (%1 SV Consumed)", round((_parameters select 3) * 100)]};
			};
		};
		
		(_parameters select 0) groupChat format ["Repairing %1... %2%3%4", (_var select CTI_STRUCTURE_LABELS) select 1, _parameters select 2, "%", _sv_consumed];
	};
	case "salvage": {
	...
	case "structure-sold-refund": {CTI_P_ChatID commandChat format ["S%1 was received from selling a %2 structure", _parameters select 0, _parameters select 1]};
	...
	case "upgrade-ended": {
		...
		if ((_parameters select 0) == CTI_UPGRADE_BARRACKS) then {
			...
			CTI_P_ChatID commandChat format ["Your units now have %1 skill", _unit_skill_val * 100];
			
			{
				if (!isPlayer _x && !isNull _x && alive _x) then {
					_x setSkill ["aimingAccuracy", _unit_skill_val]; // Set accuracy
					_x setSkill ["aimingShake", _unit_skill_val]; // Set weapon sway handling
					_x setSkill ["aimingSpeed", _unit_skill_val]; // Set aiming speed
					_x setSkill ["reloadSpeed", _unit_skill_val]; // Max out reload speed
					_x setSkill ["spotDistance", _unit_skill_val]; // Set detection distance
					_x setSkill ["spotTime", _unit_skill_val]; // Set detection time
					_x setSkill ["courage", _unit_skill_val]; // Never retreat
					_x setSkill ["commanding", _unit_skill_val]; // Communication skills
					_x setSkill ["general", _unit_skill_val];
				};
	...
	case "rank-up": {
		...
		hint parseText format ["<t align='center'>Rank Up<t size='1.3'> | </t> <t size='1.2' color='#FCBE18'>%1</t></t><br /><br /><t><t color='#FCBE18'>%2</t>", _parameters select 1, _parameters select 2];
		playMusic "LeadTrack03a_F_EPA";
	};
	case "econ-alert": {
		hint parseText format ["<t align='center'>INTEL<t size='1.3'></t> <br /><br /><t><t color='#FCBE18'>%1</t> has captured <t color='#FCBE18'>%2%3</t> of required towns to win.</t>", _parameters select 0, _parameters select 1, "%"];
	};
	...
	case "fire-support-request": {
		_requester = _parameters select 0;
		_name = localize (_parameters select 1);
		_grid = _parameters select 2;
		_dist = _parameters select 3;
		_dir = localize (_parameters select 4);
		_request = localize (_parameters select 5);
		// Localized
		_requester globalChat format[format ["%1. %2", localize "str_a3_target____1___grid___3", "- %2 m - %5 %6"], _name, _dist, _grid, _grid, _dir, _request];
		//Target: %NameSound from Grid: %Grid. Distance: %Distance. Direction: %Bearing
	};
	case "hint-holster": {
		[["CTI_Common", "CTI_Holstering"], nil, "", nil, "", true, profileNamespace getVariable "CTI_PERSISTENT_HINTS", true, nil] spawn BIS_fnc_advHint;
	};
	case "hint-hq-destroyed" : {
		[["CTI_Base", "CTI_HQRepair", "CTI_HQRepair_Destroyed"], nil, "", nil, "", true, profileNamespace getVariable "CTI_PERSISTENT_HINTS", true, nil] spawn BIS_fnc_advHint;
	};
	case "hint-commander" : {
		[["CTI_Common", "CTI_Command", "CTI_Command_VoteEnd"], nil, "", nil, "", true, profileNamespace getVariable "CTI_PERSISTENT_HINTS", true, nil] spawn BIS_fnc_advHint;
	};
	case "hint-subordinate" : {
		[["CTI_Common", "CTI_Subordinate"], nil, "", nil, "", true, profileNamespace getVariable "CTI_PERSISTENT_HINTS", true, nil] spawn BIS_fnc_advHint;
	};
	case "hint-town-camp-capture" : {
		[["CTI_Towns", "CTI_Town", "CTI_Town_CampCaptured"], nil, "", nil, "", true, profileNamespace getVariable "CTI_PERSISTENT_HINTS", true, nil] spawn BIS_fnc_advHint;
	};
};