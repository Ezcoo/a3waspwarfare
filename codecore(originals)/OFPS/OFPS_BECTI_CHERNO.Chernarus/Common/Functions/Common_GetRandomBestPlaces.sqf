/*
  ...
  # PARAMETERS #
    ...
    6	[Boolean]: True if a single position should be returned, false for all results
		7 [Number]: Min distance from center
	
  # RETURNED VALUE #
	[Array]: One or multiple positions depending on the Boolean parameter
	
  # SYNTAX #
	[CENTER, RADIUS, LAYOUT, PRECISION, SOURCECOUNT, VALIDFLOOR, RETURNONE, MINIMUMDIST] call CTI_CO_FNC_GetRandomBestPlaces
...
params ["_position", ["_radius", 200], ["_layout", "meadow"], ["_precision", 8], ["_source_count", 3], ["_valid_floor", 0.7], ["_return_one", true], ["_min_dist", 0]];
...
if (_min_dist > 0) then {
	_positions_full = _positions;
	_positions = [];
	{
		if ((_position distance _x) > _min_dist) then {_positions pushBack _x};
	} forEach _positions_full;
};
