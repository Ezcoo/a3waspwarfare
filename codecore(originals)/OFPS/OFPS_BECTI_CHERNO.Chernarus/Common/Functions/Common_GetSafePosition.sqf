...
for '_i' from 1 to _passes do {
	...
	if (count (nearestTerrainObjects [_center_ran, _filter, _distance_min, false]) < 1 && !surfaceIsWater _center_ran) then {
		...
		// Make sure it's not inside something
		if (_is_clear) then {
			if !(lineIntersectsSurfaces [AGLtoASL _center_ran, AGLtoASL _center_ran vectorAdd [0, 0, _distance_min], objNull, objNull, false, 1, "GEOM", "NONE"] isEqualTo []) then {_is_clear = false;};
		};
		if (_is_clear) then {
			_breakout = true;
