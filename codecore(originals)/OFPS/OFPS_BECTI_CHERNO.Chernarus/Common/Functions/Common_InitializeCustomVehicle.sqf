...
switch (_script) do {
	case "salvager": {
		if (CTI_IsServer) then {(_vehicle) execVM "Server\FSM\update_salvager.sqf"} else {["salvager", _vehicle] remoteExec ["CTI_PVF_SRV_RequestHandleSpecialVehicle", CTI_PV_SERVER]}
	};
	case "salvager-independent": {
		...
		[_side, _vehicle] execVM "Server\FSM\update_salvager_independent.sqf";
	};
	case "service-repairtruck": {
		_vehicle setVariable ["cti_spec", CTI_SPECIAL_REPAIRTRUCK, true]; 
		if (CTI_IsServer) then {[_vehicle, _side] execVM "Server\FSM\update_repairtruck.sqf"} else {["repair", [_vehicle, _side]] remoteExec ["CTI_PVF_SRV_RequestHandleSpecialVehicle", CTI_PV_SERVER]}
	};
	...
	case "service-all": {_vehicle setVariable ["cti_spec", [CTI_SPECIAL_GEAR, CTI_SPECIAL_AMMOTRUCK, CTI_SPECIAL_FUELTRUCK, CTI_SPECIAL_REPAIRTRUCK, CTI_SPECIAL_MEDICALVEHICLE], true]};
	case "ecm": 
	{
		_vehicle setVariable ["ecm", false, true];
		_vehicle setVariable ["cti_spec", CTI_SPECIAL_ECM, true];
	};
};