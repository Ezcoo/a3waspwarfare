...
[] spawn {
	while {al_snowstorm_om} do {
		...
		sleep 600+random ambient_sounds_al;
	};
};
...
if (terrain_type_vanilla) then {
	...
	//check for local fog on -- value > 0
	if (fulg_p_fog_radius > 0) then {
		...
		_alias_local_fog setParticleCircle [fulg_p_fog_radius, [5, 5, 0]];
		_alias_local_fog setParticleRandom [15, [0.25, 0.25, 0], [1, 1, 0], 1, 1, [0, 0, 0, 0.1], 0, 0];
	...
	if (fulg_p_fog_radius > 0) then {deletevehicle _alias_local_fog;};
	deletevehicle _fulg_nea; 
	};
