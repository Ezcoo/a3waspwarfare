...
private ["_limit", "_ratio", "_safe_range", "_sideID", "_spawn_range", "_teams_infantry", "_teams_queue", "_teams_vehicles", "_tvar", "_vvar","_is_naval","_spawn_max_ai","_spawn_min_ai","_spawn_town_sv","_active_units","_town_groups","_valid_groups","_total","_current","_active_squads","_live","_team","_group","_position","_use_default","_positions_building","_building","_has_vehicles","_position_ran","_return","_men","_vehicles"];

_sideID = (_side) call CTI_CO_FNC_GetSideID;
_is_naval = false;
if !(isNil {_town getVariable "cti_naval"}) then { _is_naval = true;};
...
_spawn_range = 250;
if (_is_naval) then {
	_safe_range = [CTI_TOWNS_NAVAL_OCCUPATION_SPAWN_SAFE_RANGE, CTI_TOWNS_NAVAL_RESISTANCE_SPAWN_SAFE_RANGE] select (_side isEqualTo resistance);
	_spawn_range = [CTI_TOWNS_NAVAL_OCCUPATION_SPAWN_RANGE, CTI_TOWNS_NAVAL_RESISTANCE_SPAWN_RANGE] select (_side isEqualTo resistance);
} else {
...
while {true} do {
	...
	if ((_total < _limit && _active_squads < _ratio) || _current < _active_units) then {
		...
		if (isNil {_town getVariable "cti_naval"}) then {
			if (([_position nearEntities ((_safe_range)/2), _side] call CTI_CO_FNC_GetAreaEnemiesCount) > 0) then {
				_use_default = true;
				if (typeName _position isEqualTo "OBJECT") then { //--- If the position was meant to be a building, we try to find a new valid building one, only applies to CTI_TOWNS_SPAWN_MODE with a value of 1
					...
					if (count _positions_building > 0) then {
						_positions_building = _positions_building call BIS_fnc_arrayShuffle;
		...
		//--- Patrol the town
		if (_is_naval) then {
			//Naval town patrol waypoints
			if (count _vehicles > 0) then {
				//Only give patrol waypoints to vehicles
				[_town, _group, _sideID] execVM "Server\FSM\town_naval_patrol.sqf";
			} else {
				//If infantry, no waypoints given as they walk off edges and end up swimming
			};
		} else {
			//Normal towns patrol waypoints
			[_town, _group, _sideID] execVM "Server\FSM\town_patrol.sqf";
		};
	...
	sleep 0.5; //--- how fast units spawn, keep in mind BIS had issues with empty groups before
};