/*
  ...
  # SYNTAX #
	...
	[POSITION, RADIUS, WATER] call CTI_CO_FNC_GetEmptyPosition
	
  # EXAMPLE #
    _empty_position = [vehicle player, 15] call CTI_CO_FNC_GetEmptyPosition
*/

params ["_object", "_radius", ["_water", false]];
...
while {_i < 1000} do {
	...
	if (_water) then {
		if (count (_tpos isFlatEmpty [15, 0, 2, 10, 2, false, objNull]) > 0 && (_tpos nearEntities 7) isEqualTo []) exitWith {_position = _tpos};
	} else {
