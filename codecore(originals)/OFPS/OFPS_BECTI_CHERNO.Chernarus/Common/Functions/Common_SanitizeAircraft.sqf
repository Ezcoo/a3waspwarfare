...
switch (missionNamespace getVariable "CTI_VEHICLES_AIR_FFAR") do {
	case 0: {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftFFAR};//--- Remove
	case 1: {if (_upgrades select CTI_UPGRADE_AIR_FFAR < 1) then {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftFFAR}};//--- Remove if not yet ugpraded
};

//--- We check the DAR loadout
switch (missionNamespace getVariable "CTI_VEHICLES_AIR_DAR") do {
	case 0: {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftDAR};//--- Remove
	case 1: {if (_upgrades select CTI_UPGRADE_AIR_DAR < 1) then {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftDAR}};//--- Remove if not yet ugpraded
};

//--- We check the AT Loadout
switch (missionNamespace getVariable "CTI_VEHICLES_AIR_AT") do {
	case 0: {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftAT};//--- Remove
	case 1: {if (_upgrades select CTI_UPGRADE_AIR_AT < 1) then {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftAT}};//--- Remove if not yet ugpraded
};

//--- We check the AA Loadout
switch (missionNamespace getVariable "CTI_VEHICLES_AIR_AA") do {
	case 0: {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftAA};//--- Remove
	case 1: {if (_upgrades select CTI_UPGRADE_AIR_AA < 1) then {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftAA}};//--- Remove if not yet ugpraded
};

//--- We check the CM Loadout
switch (missionNamespace getVariable "CTI_VEHICLES_AIR_CM") do {
	case 0: {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftCM};//--- Remove
	case 1: {if (_upgrades select CTI_UPGRADE_AIR_CM < 1) then {[_vehicle,_side] call CTI_CO_FNC_SanitizeAircraftCM}};//--- Remove if not yet ugpraded
};