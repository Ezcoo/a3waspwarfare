private ["_cpt", "_faction", "_camo", "_items", "_prices", "_upgrades"];
...
for '_i' from 0 to count(_items)-1 do {
	...
	_enabled = _item select 0;
	_name = _item select 1;
	_classname = _item select 2;
	_location = _item select 3;
	_upgradelevel = _item select 4;
	_price = _item select 5;
	_camo = _item select 6;
	_type = _item select 7;
	_pushtogear = true;
	_item = _classname;
	...
	if !(_config_type isEqualTo "nil") then { //--- Make sure that it exists
		if (isNil {missionNamespace getVariable _item}) then {
			switch (_config_type) do { //--- Each item is handled differently
				case "CfgGlasses": {
					missionNamespace setVariable [format["cti_%1", _item], [[_upgradelevel, _price, _name, _location, _enabled,_mod], "Goggles", _config_type, _type, _camo]];
				...
				case "CfgMagazines": {
					missionNamespace setVariable [format["cti_%1", _item], [[_upgradelevel, _price, _name, _location, _enabled,_mod], "Magazines", _config_type, _type, _camo]];
				...
				case "CfgVehicles": {
					if (getNumber(configFile >> _config_type >> _item >> 'isbackpack') isEqualTo 1) then { //--- Make sure that this is a backpack
						missionNamespace setVariable [format["cti_%1", _item], [[_upgradelevel, _price, _name, _location, _enabled,_mod], "Backpack", _config_type, _type, _camo]];
				...
				case "CfgWeapons": {
					...
					missionNamespace setVariable [format["cti_%1", _item], [[_upgradelevel, _price, _name, _location, _enabled,_mod], _item_type, _config_type, _type, _camo]];
					_cpt = _cpt + 1;
