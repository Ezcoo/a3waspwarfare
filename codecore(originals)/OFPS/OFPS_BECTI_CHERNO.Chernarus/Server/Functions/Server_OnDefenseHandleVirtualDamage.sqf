...
if (_virtual_damages >= 1 || !(alive _damaged)) then {
	...
	if (CTI_Log_Level >= CTI_Log_Information) then {
		["INFORMATION", "FILE: Server\Functions\Server_OnDefenseHandleVirtualDamage.sqf", format["A [%1] structure from side [%2] has been destroyed (virtual damages) by [%3]", _var select CTI_DEFENSE_LABEL, _side, _shooter]] call CTI_CO_FNC_Log;
	};
	//--- Check if the defense has a ruin model attached (we don't wana have a cemetery of wrecks)
	_ruins = "";
	if((typeName (_var select CTI_DEFENSE_CLASS)) == "ARRAY") then {
