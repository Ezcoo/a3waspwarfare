...
if (_was_deployed) then {
	...
	// Mobilize the HQ 
        _hq = [missionNamespace getVariable Format["CTI_%1_HQ", _side], (position _killed), (direction _killed), _side, true, false, false, "", objNull, false] call CTI_CO_FNC_CreateVehicle;
		_hq setVariable ["cti_gc_noremove", true]; //--- HQ wreck cannot be removed nor salvaged

	while { isInRemainsCollector _hq } do
	{
		sleep 1;
	};
