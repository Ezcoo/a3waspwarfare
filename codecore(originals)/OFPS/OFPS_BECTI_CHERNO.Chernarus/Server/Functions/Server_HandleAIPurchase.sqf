...
private ["_cost", "_direction", "_distance", "_funds", "_model", "_net", "_position", "_req_time_out", "_script", "_sideID", "_var", "_vehicle", "_var_classname","_var_crew_classname","_process","_customid","_crew","_units"];

//--- First of all, make sure that we don't go "softly" above the AI group size
_process = true;
if !(typeName _req_target isEqualTo "SIDE")  then {_process = false};
...
if !(_model isKindOf "Man") then { //--- Add the vehicle crew cost if applicable
	...
	if !(isNil '_var_crew_classname') then { _cost = _cost + ((count(_model call CTI_CO_FNC_GetVehicleTurrets)+1) * (_var_crew_classname select CTI_UNIT_PRICE)) };
};
...
if !(_process) exitWith {[_req_seed, _req_classname, _req_target, _factory] call CTI_SE_FNC_OnClientPurchaseComplete};
if !(typeName _req_target isEqualTo "SIDE") exitWith { [_req_seed, _req_classname, _req_target, _factory] call CTI_SE_FNC_OnClientPurchaseComplete };
...
if (_model isKindOf "Man") then {
	...
	//check ordinance for upgrades
	[_vehicle,_sideID] call CTI_CO_FNC_SanitizeVehicle;
