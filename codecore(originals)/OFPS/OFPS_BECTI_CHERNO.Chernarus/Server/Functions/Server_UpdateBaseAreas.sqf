...
if ((_side) call CTI_CO_FNC_IsHQDeployed) then {
	_mhq = (_side) call CTI_CO_FNC_GetSideHQ;
	_pos = getPos _mhq;
	_pos = [_pos select 0, _pos select 1];
	_structures_positions pushBack _pos;
};
...
_delete_pos = [];
{
	...
	if (_closest distance _x > CTI_BASE_AREA_RANGE) then {
		_need_update = true; 
		_delete_pos pushBack _x; 
		_areas set [_forEachIndex, "!nil!"];
	};
...
if (_need_update) then {
	...
	//--- Announce base area removal
	["base-area-removed"] remoteExec ["CTI_PVF_CLT_OnMessageReceived", _side];
