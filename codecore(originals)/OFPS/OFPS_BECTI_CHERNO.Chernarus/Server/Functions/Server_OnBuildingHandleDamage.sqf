...
switch (true) do {
	...
	// D30s do no damage 
	case (typeOf _ammo in ["CUP_Sh_122_AT", "CUP_Sh_122_HE", "CUP_Sh_122_WP", "CUP_Sh_122_LASER", "CUP_Sh_122_SMOKE"] ): {
		_dmg_mutliplier = -1;
		_dmg_max = 0;
	};
...
if (CTI_BASE_DISPLAY_HINT isEqualTo 1) then{
	...
	if (alive _damaged && !(side _shooter in [_side, sideEnemy]) && _damage >= 0.02) then {
