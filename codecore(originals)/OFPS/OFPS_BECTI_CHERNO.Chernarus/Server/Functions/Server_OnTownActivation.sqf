...
params ["_town", "_side"];
private ["_return", "_town_groups", "_town_positions", "_town_teams", "_var_side","_delegated"];
...
//--- Create the town static defenses according to the town situation if needed
if !(isNil {_town getVariable "cti_town_hasstatics"}) then {
	if (CTI_Log_Level >= CTI_Log_Information) then {
		["INFORMATION", "FILE: Server\Functions\Server_OnTownActivation.sqf", format["Creating Town Static Defenses for side [%1] in town [%2]", _side, _town]] call CTI_CO_FNC_Log;
	};
	
	[_town, _side] call CTI_SE_FNC_CreateStaticTownDefenses;
};
