...
private ["_candidates", "_candidates_count", "_delegated", "_delegation_table","_index_hc","_owner_id","_hc_entity","_hc_entity","_uid","_sub_teams","_sub_groups","_sub_positions","_locality"];
...
if !(isNil '_candidates') then {
	...
	if (count _candidates > 0 && count _groups > 0) then {
		...
		if (count _candidates > 1) then {_candidates = (_candidates) call BIS_fnc_arrayShuffle};
		...
		for '_i' from 0 to count(_teams)-1 do {
			...
			_index_hc = [_index_hc + 1, 0] select (_index_hc+1 > _candidates_count);
		};
