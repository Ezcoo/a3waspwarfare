...
with missionNamespace do {
    ...
    //--- The client(and sometimes server) send a database update request
	CTI_PVF_SRV_DatabaseUpdate = { _this spawn CTI_SE_FNC_DatabaseUpdate };
	//--- LogNewHeadlessClient on DB
	CTI_PVF_SRV_LogNewHeadless = {
		if (canSuspend) then {
			waitUntil {
				!(isNil "GameID");
			};
		};
		call compile ("extDB3" callExtension format["0:logger:LogNewHeadlessClient:%1", GameID]);
	};
	...
	CTI_PVF_SRV_OnArtilleryPieceTracked = {
		_artillery = _this;
		_artillery addEventHandler ["Fired", {[_this select 0, _this select 4, _this select 6] spawn CTI_SE_FNC_OnArtilleryFired}];
	};
	
	/*CTI_PVF_SRV_AddCRAMFireEH = { 
		_unit = _this; 
		_unit addEventHandler ["FiredMan",{_this spawn FNC_CRAMControl_AttackerFiredEH_SERVER}]; 

		_hcs = missionNamespace getVariable "CTI_HEADLESS_CLIENTS"; 
		// Run on server or HC 
		if ( !isNil '_hcs' && {count _hcs > 0} ) then { 
			_hc = (_hcs select 0) select 0; 
			_this remoteExec ["CTI_PVF_HC_AddCRAMFireEH", _hc]; 
		} else { 
			_unit addEventHandler ["Fired",{_this spawn FNC_CRAMControl_AttackerFiredEH_SERVER}]; 
		};
	}; */
   
	
	CTI_PVF_SRV_OnCRAMTargetableFired = {
		_shooterSide = _this select 0;
		_projectile = _this select 1;
		//_projectile = nearestObject [_this select 1, _this select 2];
		if (isNull _projectile) then {
			diag_log ("Server received null targetable");
		};
		
		{
			_targets = missionNamespace getVariable [format ["CRAM_Targets_%1", _x], []];
			_targets pushBackUnique _projectile;
			missionNamespace setVariable [format ["CRAM_Targets_%1", _x], _targets];
		} forEach ([west, east, resistance] - [_shooterSide]);
		...
		if ( !isNil '_hcs' && {count _hcs > 0} ) then {
			_hc = (_hcs select 0) select 0;
			[_shooterSide, position _projectile, typeOf _projectile] remoteExec ["CTI_PVF_HC_OnCRAMTargetableFired", _hc];
		} else {
			{
				_targets = missionNamespace getVariable [format ["CRAM_Targets_%1", _x], []];
				_targets pushBackUnique _projectile;
	...
	CTI_PVF_SRV_RequestCommanderPassoff = {
		private ["_logic", "_name", "_side", "_team"];
		...
		//--- Make sure that no vote is running
		if (_logic getVariable "cti_votetime" <= 0) then {
			//--- Make sure the person doing this is still current commander
			if (isRemoteExecuted && !(remoteExecutedOwner isEqualTo groupOwner (_side call CTI_CO_FNC_GetSideCommanderTeam))) exitWith {};
			
			
			_logic = (_side) call CTI_CO_FNC_GetSideLogic;
			
			
			_currentComm = (_side call CTI_CO_FNC_GetSideCommanderTeam);
			_newComm = _currentComm getVariable "cti_vote";
			
			// Make sure the new comm is still here
			if (!isNull _newComm && !isPlayer leader _newComm) exitWith {};

			//--- Set the new commander
			_logic setVariable ["cti_commander_team", _newComm, true];

			if (isNull _newComm) then {
				
				if (_logic getVariable "cti_ai_commander") then {_logic setVariable ["cti_ai_commander", false]};
				_hq = (_side) call CTI_CO_FNC_GetSideHQ;
				if (alive _hq) then {
					_hq setOwner (groupOwner _newComm); //--- Transfer HQ locality
				};
			};
			["commander-vote-end", _newComm] remoteExec ["CTI_PVF_CLT_OnMessageReceived", _side];
		};
	};
	...
	CTI_PVF_SRV_RequestHandleSpecialVehicle = { 
		...
		switch (_action) do {
			case "repair": {_param execVM "Server\FSM\update_repairtruck.sqf"};
			case "salvager": {_param execVM "Server\FSM\update_salvager.sqf"};
		};
	};
	...
	CTI_PVF_SRV_RequestVehicleHullRepair = {
		private ["_damages", "_locked", "_repair", "_vehicle"];
		_vehicle = _this select 0;
		_hitPoints_repair = _this select 1;
		_repair_ammount = CTI_TOOLKIT_HITPOINT_REPAIR_AMMOUNT;
		...
		if (local _vehicle) then {
			...
			[_vehicle, _hitPoints_repair] remoteExec ["CTI_PVF_CLT_RequestVehicleHullRepair", owner _vehicle];
		};
	};
};