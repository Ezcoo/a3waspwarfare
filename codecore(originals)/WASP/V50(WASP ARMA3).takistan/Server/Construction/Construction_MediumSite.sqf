...
_constructed = ([_position,_direction,WFBE_MEDIUM_SITE_1_OBJECTS] Call WFBE_SE_FNC_CreateObjectsFromArray);
...
_constructed = _constructed + ([_position,_direction,WFBE_MEDIUM_SITE_2_OBJECTS] Call WFBE_SE_FNC_CreateObjectsFromArray);
...
_constructed = _constructed + ([_position,_direction,WFBE_MEDIUM_SITE_3_OBJECTS] Call WFBE_SE_FNC_CreateObjectsFromArray);
...
_defenses = [_site, missionNamespace getVariable format ["WFBE_NEURODEF_%1_WALLS", _rlType]] call WFBE_SE_FNC_CreateDefenseTemplate;
_site setVariable ["WFBE_Walls", _defenses];
};

[_side, "Constructed", ["Base", _site]] Spawn WFBE_SE_FNC_SideMessage;
...
if (!IsNull _site) then {
	...
	_site addEventHandler ["hit",{_this Spawn WFBE_SE_FNC_BuildingDamaged}];
	_site addEventHandler ['handleDamage',{[_this select 0,_this select 2,_this select 3] Call WFBE_SE_FNC_BuildingHandleDamages}];
	Call Compile Format ["_site AddEventHandler ['killed',{[_this select 0,_this select 1,'%1'] Spawn WFBE_SE_FNC_BuildingKilled}];",_type];
