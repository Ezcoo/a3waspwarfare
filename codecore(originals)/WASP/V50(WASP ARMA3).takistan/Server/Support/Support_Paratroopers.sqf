...
for '_i' from 1 to _vehicle_count do {
	//--- Spawn the vehicle.
	_vehicle = createVehicle [missionNamespace getVariable Format ["WFBE_%1PARACARGO",str _side],(_ranPos select _ran), [], _dir, "FLY"];
...
[str _side, 'VehiclesCreated', _built] Call WFBE_CO_FNC_UpdateStatistics;
...
[str _side,'UnitsCreated', _built] Call WFBE_CO_FNC_UpdateStatistics;

//--- Tell the group to move.
[_grp, _destination, "MOVE", 10] Call WFBE_SE_FNC_AIMoveTo;
...
if (_greenlight) then {
	...
	[_grp, (_ranPos select _ran), "MOVE", 5] Call WFBE_SE_FNC_AIMoveTo;
