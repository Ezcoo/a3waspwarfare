	...
	if (_victory == 0) then {
		{
			...
			_towns = (_x) Call WFBE_CO_FNC_GetTownsHeld;
	
			_factories = 0;
			{
				_factories = _factories + count([_side,missionNamespace getVariable Format ["WFBE_%1%2TYPE",_side,_x], _structures] Call WFBE_CO_FNC_GetFactories);
			} forEach ["BARRACKS","LIGHT","HEAVY","AIRCRAFT"];

			if (!(alive _hq)  && _factories == 0 || _towns == _total && !WFBE_GameOver) then {
				["endgame", (_x) Call WFBE_CO_FNC_GetSideID] remoteExecCall ["WFBE_CL_FNC_HandleSpecial"];
