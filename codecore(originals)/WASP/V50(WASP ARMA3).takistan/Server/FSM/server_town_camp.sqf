			...
			if(_west > 0 || _east > 0 || _resistance > 0) then{
				...
				if(_captured)then{
					...
					if (_sideID != WFBE_C_UNKNOWN_ID) then {
						if (missionNamespace getVariable Format ["WFBE_%1_PRESENT",_side]) then {[_side,"LostAt",["Strongpoint",_town]] Spawn WFBE_SE_FNC_SideMessage};
					};

					if (missionNamespace getVariable Format ["WFBE_%1_PRESENT",_newSide]) then {[_newSide,"CapturedNear",["Strongpoint",_town]] Spawn WFBE_SE_FNC_SideMessage};

					_camp setVariable ["sideID",_newSID,true];
					_flag setFlagTexture (missionNamespace getVariable Format["WFBE_%1FLAG",str _side]);

					[_camp,_newSID,_sideID] remoteExecCall ["WFBE_CL_FNC_CampCaptured"];
				};
			};
		}else{};
