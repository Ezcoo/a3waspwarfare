        ...
        if(_west > 0 || _east > 0 || _resistance > 0) then {
            ...
            if(_town_capture_mode == 2) then {
                ...
                if (_west > 0 && west in WFBE_PRESENTSIDES) then {
                    if (_totalCamps != ([_location,west] Call WFBE_CO_FNC_GetTotalCampsOnSide)) then {_skip = true};
                };
                if (_east > 0 && east in WFBE_PRESENTSIDES) then {
                    if (_totalCamps != ([_location,east] Call WFBE_CO_FNC_GetTotalCampsOnSide)) then {_skip = true};
                };
                if (_resistance > 0 && resistance in WFBE_PRESENTSIDES) then {
                    if (_totalCamps != ([_location,resistance] Call WFBE_CO_FNC_GetTotalCampsOnSide)) then {_skip = true};
                };
            };
            ...
            if !(_skip) then {
                ...
                if (_sideID != WFBE_C_UNKNOWN_ID) then {
                    if (_activeEnemies > 0 && time > _timeAttacked && (missionNamespace getVariable Format ["WFBE_%1_PRESENT",_side])) then {_timeAttacked = time + 60;[_side, "IsUnderAttack", ["Town", _location]] Spawn WFBE_SE_FNC_SideMessage};
                };
			...
			["INFORMATION", Format ["server_town.sqf: Town [%1] was captured by [%2] From [%3].", _location, _newSide, _side]] Call WFBE_CO_FNC_LogContent;
                ...
                if (_sideID != WFBE_C_UNKNOWN_ID) then {
                    if (missionNamespace getVariable Format ["WFBE_%1_PRESENT",_side]) then {[_side, "Lost", _location] Spawn WFBE_SE_FNC_SideMessage};
                };

                if (missionNamespace getVariable Format ["WFBE_%1_PRESENT",_newSide]) then {[_newSide, "Captured", _location] Spawn WFBE_SE_FNC_SideMessage};

                _location setVariable ["sideID",_newSID,true];

                [_location, _sideID, _newSID] remoteExecCall ["WFBE_CL_FNC_TownCaptured"];

                [_location, _sideID, _newSID] Spawn WFBE_SE_FNC_SetCampsToSide;
