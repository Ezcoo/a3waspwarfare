...
if(count _alives == 0) then{
	{
		...
		if !(isNull _building) then {
			...
			[_IDS,_building,_x,_side,_inf_group,_vehiSlots,_additionalSlotsAmount] call WFBE_SE_FNC_Server_ResBuyUnit;
		};
	} forEach _template;
};
...
while{!_end}do{
	...
	_alives = (units _inf_group) Call WFBE_CO_FNC_GetLiveUnits;
	if(count _alives > 0)then{
    ...
    _nearTowns = [];
    _near = [];
    _allSortedTownsByDistance = [getPosATL (leader _inf_group),towns] Call WFBE_CO_FNC_SortByDistance;
    ...
    [_inf_group,_nearTowns,400,'LINE'] Call WFBE_SE_FNC_AITownPatrol;
