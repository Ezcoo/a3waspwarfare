...
_units = units _team;
_units = _units + ([_team,false] Call WFBE_CO_FNC_GetTeamVehicles) - [_hq];
...
if !(isNull (_commander)) then {
	if (_team == _commander) then {
		...
		['CommanderDisconnected'] remoteExecCall ["WFBE_CL_FNC_LocalizeMessage", _side];
	};
};

[leader _team, _team getVariable "wsw_role"] call WSW_fnc_svrResetRoles;