...
if (count _to_upgrade > 0) then {
	...
	_upgrade_cost_array = missionNamespace getVariable Format["WFBE_C_UPGRADES_%1_COSTS", _side];
	if(!(isNil '_upgrade_cost_array') && count _upgrade_cost_array > 0)then{
        ...
        _funds = _side Call WFBE_SE_FNC_GetAICommanderFunds;
        ...
        if (_can_upgrade) then {
            ...
            //--- Deduct.
            [_side,-(_cost select 0)] Call WFBE_SE_FNC_ChangeAICommanderFunds;
            [_side,-(_cost select 1)] Call WFBE_CO_FNC_ChangeSideSupply;
        };
