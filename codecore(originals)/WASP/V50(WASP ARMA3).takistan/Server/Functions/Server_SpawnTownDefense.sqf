...
if (count _kinds > 1) then {
	...
	while {true} do {
		...
		if !(isNil '_kind') then {_defense = _kind select floor(random count _kind);} else {_nils pushBack (_kinds select _random); _kinds = [_kinds, [_random]] Call WFBE_CO_FNC_ArrayShift;};
...
if (_defense != "") then {
	if(!(_defense in WFBE_C_STATIC_DEFENCE_FOR_COMPOSITIONS))then {
		Private["_entitie"];
