...
_delay = if (count _this > 1) then {_this select 1} else {
    if (typeOf _vehicle in (missionNamespace getVariable Format['WFBE_%1AMBULANCES',str (side _vehicle)])) then {
        (missionNamespace getVariable "WFBE_C_UNITS_EMPTY_TIMEOUT")*2
    };
};

if(isNil '_delay')then{
    _delay = missionNamespace getVariable "WFBE_C_UNITS_EMPTY_TIMEOUT";
};
