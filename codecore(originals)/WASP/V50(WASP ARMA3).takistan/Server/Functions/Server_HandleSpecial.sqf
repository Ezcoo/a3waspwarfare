...
switch (_args select 0) do {
		...
		if (alive _player) then {
			if (alive leader _group) then {
				if (isPlayer leader _group) then {
					...
					['group-join-request', _player] remoteExecCall ["WFBE_CL_FNC_HandleSpecial", leader _group];
					...
					if (isNil {_group getVariable "wfbe_uid"}) then { //--- Ensure that the group is ai-controlled.
						...
						["group-join-accept", _group] remoteExecCall ["WFBE_CL_FNC_HandleSpecial", _player];
					};
				};
			};
		};
	...
	case "ICBM": {
		...
		[_base] Spawn WFBE_CO_FNC_NukeDammage;
	};
	...
	case "repair-camp": {
		...
		if (_camp_sideID != _repairSideID) then {
			...
			[_logic, _repairSideID, _camp_sideID, true] remoteExecCall ["WFBE_CL_FNC_CampCaptured"];
		};
	};
	case "destroy-camp": {
		Private ["_camp_sideID","_logic","_repairSideID","_townModel"];
		...
		(_logic getVariable 'wfbe_camp_bunker') allowDamage  true;
		(_logic getVariable 'wfbe_camp_bunker') setDamage 1;
		
		sleep 1.3;
		deleteVehicle (_logic getVariable 'wfbe_camp_bunker');
	};
};