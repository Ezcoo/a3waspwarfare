...
[_azi, _lfResBasePositions] Spawn {
    ...
    if (count _lfResBasePositions > 0) then {
        ...
        _objs   = missionNamespace getVariable "WFBE_NEURODEF_RESISTANCE_LF";
        for [{_i = 0},{_i < count _positionsToBuildLF},{_i = _i + 1}] do {
            ...
            for "_j" from 0 to ((count _objs) - 1) do {
                ...
                if(_type in WFBE_C_STATIC_DEFENCE_FOR_COMPOSITIONS)then{
            	...
            	if(_type in WFBE_C_GARBAGE_OBJECTS)then{ _newObj enableSimulation false; };

            	if(_type == ((missionNamespace getVariable Format["WFBE_%1STRUCTURENAMES",str resistance]) select 2)) then {
            	    [_newObj, resistance, _type, _i] spawn WFBE_SE_FNC_InitResLight
            	};
            ...
            [_defences, _pos] call WFBE_SE_FNC_ManningOfResBaseDefense;
        };
    };
};
[_azi, _hfResBasePositions] Spawn {
    ...
    if (count _hfResBasePositions > 0) then {
        ...
        _objs   = missionNamespace getVariable "WFBE_NEURODEF_RESISTANCE_HF";
        for [{_i = 0},{_i < count _positionsToBuildHF},{_i = _i + 1}] do {
            ...
            for "_j" from 0 to ((count _objs) - 1) do {
                ...
                if(_type == ((missionNamespace getVariable Format["WFBE_%1STRUCTURENAMES",str resistance]) select 4)) then {
                    [_newObj, resistance, _type, _i] spawn WFBE_SE_FNC_InitResHeavy
                };
...
[_azi, _afResBasePositions] Spawn {
    ...
    if (count _afResBasePositions > 0) then {
        ...
        _objs   = missionNamespace getVariable "WFBE_NEURODEF_RESISTANCE_AF";
        for [{_i = 0},{_i < count _positionsToBuildAF},{_i = _i + 1}] do {
            ...
            for "_j" from 0 to ((count _objs) - 1) do {
                ...
                if(_type == ((missionNamespace getVariable Format["WFBE_%1STRUCTURENAMES",str resistance]) select 5)) then {
                    [_newObj, resistance, _type, _i] spawn WFBE_SE_FNC_InitResAir
                };
