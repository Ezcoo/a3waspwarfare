...
if (_upgrade_isplayer) then {
	...
	['upgrade-started', _upgrade_id, _upgrade_level + 1] remoteExecCall ["WFBE_CL_FNC_HandleSpecial", _side];
...
[_side, "NewIntelAvailable"] Spawn WFBE_SE_FNC_SideMessage;

['upgrade-complete', _upgrade_id, _upgrade_level + 1] remoteExecCall ["WFBE_CL_FNC_HandleSpecial", _side];
