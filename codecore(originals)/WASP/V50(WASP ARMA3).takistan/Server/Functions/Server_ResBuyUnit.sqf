...
if (_unitType isKindOf "Man") then {
	if (count _HC > 0) then {
		...
		[_side, _unitType, _position, _team, _dir] Call WFBE_SE_FNC_DelegateAIHeadless;
	}else{
	    _soldier = [_unitType,_team,_position,_sideID] Call WFBE_CO_FNC_CreateUnit;
        //--- Infantry can use the team vehicles as cargo.
        _vehicle = [_team,true] Call WFBE_CO_FNC_GetTeamVehicles;
    ...
    if (count _HC > 0) then {
        ...
        [_side, _unitType, _position, _team, _dir, _special] Call WFBE_SE_FNC_DelegateAIHeadless;
    }else{
        _crew = missionNamespace getVariable Format["WFBE_%1RESSOLDIER",_sideText];

        if (_unitType isKindOf "Tank") then {_crew = missionNamespace getVariable Format ["WFBE_%1RESCREW",_sideText]};
        if (_unitType isKindOf "Air") then {_crew = missionNamespace getVariable Format ["WFBE_%1RESPILOT",_sideText]};
