...
if (WFBE_C_ENVIRONMENT_WEATHER_OVERCAST ==-1) then { 
	_overcast_setting = random 1 
} else { 
	_overcast_setting = WFBE_C_ENVIRONMENT_WEATHER_OVERCAST / 100
};

if (WFBE_C_ENVIRONMENT_WEATHER_FOG ==-1) then { 
	_fog_setting = random 1 
} else { 
	_fog_setting = WFBE_C_ENVIRONMENT_WEATHER_FOG / 100 
};

if (WFBE_C_ENVIRONMENT_WEATHER_WIND ==-1) then { 
	_wind_setting = random 10 
} else { 
	_wind_setting = WFBE_C_ENVIRONMENT_WEATHER_WIND / 10 
};

if (WFBE_C_ENVIRONMENT_WEATHER_WAVES ==-1) then { 
	_waves_setting = random 1 
} else { 
	_waves_setting = WFBE_C_ENVIRONMENT_WEATHER_WAVES / 100 
};
...
while {!WFBE_GameOver} do {
	...
	while {true} do {
		...
		sleep WFBE_C_ENVIRONMENT_WEATHER_TRANSITION;
		setWind [random [-10,0,10], random [-10,0,10], true];
		_nexttime setWindStr _wind_setting;
	}
}