...
{lbAdd[17008,_x]} forEach (missionNamespace getVariable Format ["WFBE_%1_ARTILLERY_DISPLAY_NAME",WFBE_Client_SideJoinedText]);
lbSetCurSel[17008,0];
...
while {alive player && dialog} do {
	if (side player != WFBE_Client_SideJoined) exitWith {deleteMarkerLocal _marker;deleteMarkerLocal _area;{deleteMarkerLocal _x} forEach _markers;closeDialog 0};
	...
	if (_currentSel != _lastSel || _forceReload) then {
        ...
        if!(isNil '_selectedRole')then{
            if(_selectedRole == WSW_UAV_OPERATOR)then{
                _currentFee = _currentFee - (_currentFee * WFBE_ADV_UAV_DISCOUNT);
            };
        };
	...
	if (WF_MenuAction == 20) then {
		...
		switch (_currentSpecial) do {
			case "ICBM": {
				...
				_textAnimHandler = [17022,localize 'STR_WF_TACTICAL_ClickOnMap',10,"ff9900"] spawn WFBE_CL_FNC_SetControlFadeAnim;
			};
			case "Paratroopers": {
				WF_MenuAction = 3;
				if !(scriptDone _textAnimHandler) then {terminate _textAnimHandler};
	...
	if (mouseButtonUp == 0) then {
		...
		if (WF_MenuAction == 3) then {
			...
			_forceReload = true;
			if !(scriptDone _textAnimHandler) then {terminate _textAnimHandler};
			[17022] Call WFBE_CL_FNC_SetControlFadeAnimStop;
			_callPos = _map posScreenToWorld[mouseX,mouseY];
			if (!surfaceIsWater _callPos) then {
				lastParaCall = time;
				-(_currentFee) Call WFBE_CL_FNC_ChangePlayerFunds;
				["Paratroops",WFBE_Client_SideJoined,_callPos,WFBE_Client_Team] remoteExecCall ["WFBE_SE_PVF_RequestSpecial",2];
		...
		if (WF_MenuAction == 8) then {
			...
			[_obj,_nukeMarker] Spawn WFBE_CO_FNC_NukeIncomming;
		};
		//--- Vehicle Paradrop.
		if (WF_MenuAction == 9) then {
			...
			-_currentFee Call WFBE_CL_FNC_ChangePlayerFunds;
			_callPos = _map PosScreenToWorld[mouseX,mouseY];
			["ParaVehi",WFBE_Client_SideJoined,_callPos,WFBE_Client_Team] remoteExecCall ["WFBE_SE_PVF_RequestSpecial",2];
		};
		//--- Ammo Paradrop.
		if (WF_MenuAction == 10) then {
			...
			-_currentFee Call WFBE_CL_FNC_ChangePlayerFunds;
			_callPos = _map PosScreenToWorld[mouseX,mouseY];
			["ParaAmmo",WFBE_Client_SideJoined,_callPos,WFBE_Client_Team] remoteExecCall ["WFBE_SE_PVF_RequestSpecial",2];
		};
	};
	...
	if (WF_MenuAction == 2) then {
		WF_MenuAction = -1;
		_units = [Group player,false,lbCurSel(17008),WFBE_Client_SideJoinedText] Call WFBE_CO_FNC_GetTeamArtillery;
		if (Count _units > 0) then {
			fireMissionTime = time;
			[GetMarkerPos "artilleryMarker",lbCurSel(17008)] Spawn WFBE_CL_FNC_RequestFireMission;
		} else {
	...
	if (WF_MenuAction == 200 || _startLoad) then {
		...
		_minRange = (missionNamespace getVariable Format ["WFBE_%1_ARTILLERY_RANGES_MIN",WFBE_Client_SideJoined]) select _index;
		_maxRange = round(((missionNamespace getVariable Format ["WFBE_%1_ARTILLERY_RANGES_MAX",WFBE_Client_SideJoined]) select _index) / (missionNamespace getVariable "WFBE_C_ARTILLERY"));

		_trackingArray = [group player,true,lbCurSel(17008),WFBE_Client_SideJoined] Call WFBE_CO_FNC_GetTeamArtillery;
	...
	if ((missionNamespace getVariable "WFBE_C_ARTILLERY") > 0 && (_requestRangedList || time - _lastArtyUpdate > 3)) then {
		...
		_i = 0;
		{
			...
			lnbAddRow [17024,[[typeOf _x, 'displayName'] Call WFBE_CO_FNC_GetConfigInfo,_text]];
			lnbSetPicture [17024,[_i,0],[typeOf _x, 'picture'] Call WFBE_CO_FNC_GetConfigInfo];
	...
	if (WF_MenuAction == 40) then {
		...
		if !(scriptDone _textAnimHandler) then {terminate _textAnimHandler};
		_textAnimHandler = [17022,localize 'STR_WF_TACTICAL_ArtilleryMinimapInfo',7,"ff9900"] spawn WFBE_CL_FNC_SetControlFadeAnim;
