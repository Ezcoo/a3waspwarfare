...
if(!(isNil '_oldpylons' )) then { 
	if (count _oldpylons > 0) then {
		_pylons = +_oldpylons;
		_rearmPrice = round((((missionNamespace getVariable _type) select QUERYUNITPRICE)/(missionNamespace getVariable "WFBE_C_UNITS_SUPPORT_REARM_PRICE"))*1.25);
...
if(!(isNil '_oldpylons' ) ) then { 
	if (count _oldpylons > 0) then {	
		...
		if(round(_rearmPrice / 2) <= Call WFBE_CL_FNC_GetPlayerFunds && (count _pylons > 0)) then {
...
while {alive player && dialog} do {
	...
	if (WF_MenuAction == 111) then {
		...
		lbClear 230034;
		{
			if(_x != "CUP_PylonPod_ANAAQ_28" && _x != "CUP_PylonPod_ALQ_131") then {
	...
	if (WF_MenuAction == 112) then {
		...
		if(_rearmPrice <= Call WFBE_CL_FNC_GetPlayerFunds && (count _pylons > 0)) then {
	...
	if (WF_MenuAction == 101) then {
		...
		if(count _pylons > 0) then {
			...
			[_veh, _pylons] call WFBE_CO_FNC_Requip_AIR_VEH;
		};
	...
	if (WF_MenuAction == 115) then {
		...
		-(round(_rearmPrice / 2)) Call WFBE_CL_FNC_ChangePlayerFunds;		
		...
		[_veh, side player] Spawn WFBE_CO_FNC_RearmVehicle;
	};
};