...
_n = 1;
{lbAdd[21002,Format["[%1] %2",_n,name (leader _x)]];_n = _n + 1} forEach WFBE_Client_Teams;
_id = WFBE_Client_Teams find WFBE_Client_Team;
lbSetCurSel[21002,_id];
_currentUnit = (player) Call WFBE_CO_FNC_GetUnitVehicle;
_currentMode = "Internal";
_currentUnit switchCamera _currentMode;
_units = (Units (group player) - [player]) Call WFBE_CO_FNC_GetLiveUnits;
...
while {true} do {
	...
	_cameraSwap = false;
	if (Side player != WFBE_Client_SideJoined || !dialog) exitWith {};
	...
	if (mouseButtonUp == 0) then {
		...
		if (count _list > 0) then {
			_objects = [];
			{if (!(_x isKindOf "Man") && side _x != WFBE_Client_SideJoined) then {if (count (crew _x) == 0) then {_objects = _objects - [_x]}};if (side _x == WFBE_Client_SideJoined) then {_objects = _objects + [_x]}} forEach _list;
			if (count _objects > 0) then {
				_currentUnit = ([_near,_objects] Call WFBE_CO_FNC_GetClosestEntity) Call WFBE_CO_FNC_GetUnitVehicle;
				_cameraSwap = true;
			};
		};
	};	
	...
	if (WF_MenuAction == 101) then {
		WF_MenuAction = -1;
		_selected = leader (WFBE_Client_Teams select (lbCurSel 21002));
		_currentUnit = (_selected) Call WFBE_CO_FNC_GetUnitVehicle;
		_n = 0;
		_units = (Units (group _selected) - [_selected]) Call WFBE_CO_FNC_GetLiveUnits;
		lbClear 21004;
		{lbAdd[21004,Format["(%1) %2",GetText (configFile >> "CfgVehicles" >> (typeOf (vehicle _x)) >> "displayName"),name _x]];_n = _n + 1} forEach _units;
		_cameraSwap = true;
	};
	...
	if (WF_MenuAction == 102) then {
		WF_MenuAction = -1;
		_currentUnit = (_units select (lbCurSel 21004)) Call WFBE_CO_FNC_GetUnitVehicle;
...
closeDialog 0;
((player) Call WFBE_CO_FNC_GetUnitVehicle) switchCamera _currentMode;