...
ctrlSetText [13010, Format [localize "STR_WF_Income",Call WFBE_CL_FNC_GetPlayerFunds,(WFBE_Client_SideJoined) Call WFBE_CL_FNC_GetIncome]];
...
while {alive player && dialog} do {
	...
	if (WF_MenuAction == 1) then {
		WF_MenuAction = -1;
		if ((_transferAmount != 0)&&((WFBE_Client_Teams select _curSel) != group player)) then {
			[(WFBE_Client_Teams select _curSel),_transferAmount] Call WFBE_CO_FNC_ChangeTeamFunds;
			-_transferAmount Call WFBE_CL_FNC_ChangePlayerFunds;
			_funds = Call WFBE_CL_FNC_GetPlayerFunds;
			if (isPlayer leader (WFBE_Client_Teams select _curSel)) then {
				['FundsTransfer',_transferAmount,name player] remoteExecCall ["WFBE_CL_FNC_LocalizeMessage", leader (WFBE_Client_Teams select _curSel)];
			};
			sliderSetRange[13007,0,_funds];
		};
	};
	...
	if (_timer > 2) then {ctrlSetText [13010, Format [localize "STR_WF_Income",Call WFBE_CL_FNC_GetPlayerFunds,(WFBE_Client_SideJoined) Call WFBE_CL_FNC_GetIncome]];_timer = 0};
	_timer = _timer + 0.05;
