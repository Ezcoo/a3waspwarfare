...
_temp = _camps;
{
	if (!(alive (_x getVariable 'wfbe_camp_bunker'))) then {_camps deleteAt _forEachIndex};
} forEach _temp;
...
if ((missionNamespace getVariable "WFBE_C_CAMPS_REPAIR_PRICE") > 0) then {
	...
	if ((Call WFBE_CL_FNC_GetPlayerFunds) < (missionNamespace getVariable "WFBE_C_CAMPS_REPAIR_PRICE")) exitWith {hint Format [parseText(localize "STR_WF_Repair_Camp_NoFunds"), (missionNamespace getVariable "WFBE_C_CAMPS_REPAIR_PRICE") - (Call WFBE_CL_FNC_GetPlayerFunds)]};

	//--- Purchase a repair.
	-(missionNamespace getVariable "WFBE_C_CAMPS_REPAIR_PRICE") Call WFBE_CL_FNC_ChangePlayerFunds;
};
...
["destroy-camp", _camp, WFBE_Client_SideID] remoteExecCall ["WFBE_SE_PVF_RequestSpecial",2];
