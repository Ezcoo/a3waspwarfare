...
commanderTeam = (WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetCommanderTeam;
	...
	if(_changeCommander) then {
		...
		if (!isNull(commanderTeam)) then {
			if (commanderTeam == Group player) then {
				...
				[Localize "STR_WF_CHAT_PlayerCommander"] Call WFBE_CL_FNC_TitleTextMessage;
				["INFORMATION", Format ["Player %1 has become a new commander in %2 team).", name player, side player]] Call WFBE_CO_FNC_LogContent;
				player addAction ["<t color='#FF0000'>"+ "RECOVER HQ" + "  " + str (missionNameSpace getVariable 'WFBE_C_BASE_HQ_REPAIR_PRICE_CASH') +"$" +"</t>", "Client\Action\Action_RepairMHQDepot.sqf", [], 1, false, true, "", "(!(alive ((WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideHQ)))&&(leader  (WFBE_Client_SideJoined call WFBE_CO_FNC_GetCommanderTeam) == leader (vehicle player))&&(typeOf cursorTarget in ['Fort_CAmp','WFBE_C_DEPOT'])&&(cursorTarget distance player < 100)"];
			} else {
				if (!isNil "HQAction") then {player removeAction HQAction};
				if (count (hcAllGroups player) > 0) then {HCRemoveAllGroups player};
			};
