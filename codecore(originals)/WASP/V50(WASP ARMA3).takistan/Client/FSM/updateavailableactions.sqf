...
_typeRepair = missionNamespace getVariable Format['WFBE_%1REPAIRTRUCKS',WFBE_Client_SideJoinedText];
...
while {!WFBE_GameOver} do {
	...
	while {time - _lastUpdate > 5 || WFBE_ForceUpdate} do {
		...
		if (_boundaries_enabled) then {
			_isOnMap = Call WFBE_CL_FNC_BoundariesIsOnMap;
			if (!_isOnMap && alive player && !WFBE_Client_IsRespawning) then {
				if !(paramBoundariesRunning) then {_handle = [] Spawn WFBE_CL_FNC_BoundariesHandleOnMap;};
			} else {
		...
		if !(isNull _base) then {
			hqInRange = if ((player distance _base < _mhqbr) && alive _base  && (side _base in [WFBE_Client_SideJoined,civilian])) then {true} else {false};
		};

		barracksInRange = if (isNull (['BARRACKSTYPE',_buildings,_purchaseRange,WFBE_Client_SideJoined,player] Call WFBE_CO_FNC_BuildingInRange)) then {false} else {true};
		gearInRange = if (isNull (['BARRACKSTYPE',_buildings,_pgr,WFBE_Client_SideJoined,player] Call WFBE_CO_FNC_BuildingInRange)) then {false} else {true};
		if !(gearInRange) then {
		...
		lightInRange = if (isNull (['LIGHTTYPE',_buildings,_purchaseRange,WFBE_Client_SideJoined,player] Call WFBE_CO_FNC_BuildingInRange)) then {false} else {true};
		heavyInRange = if (isNull (['HEAVYTYPE',_buildings,_purchaseRange,WFBE_Client_SideJoined,player] Call WFBE_CO_FNC_BuildingInRange)) then {false} else {true};
		aircraftInRange = if (isNull (['AIRCRAFTTYPE',_buildings,_purchaseRange,WFBE_Client_SideJoined,player] Call WFBE_CO_FNC_BuildingInRange)) then {false} else {true};
		serviceInRange = if (isNull (['SERVICEPOINTTYPE',_buildings,_spr,WFBE_Client_SideJoined,player] Call WFBE_CO_FNC_BuildingInRange)) then {false} else {true};
		...
		_checks = [WFBE_Client_SideJoined, missionNamespace getVariable Format ["WFBE_%1AARADARTYPE",WFBE_Client_SideJoinedText],_buildings] Call WFBE_CO_FNC_GetFactories;
		if (count _checks > 0) then {antiAirRadarInRange = true;} else {antiAirRadarInRange = false;};
		
		_checks = [WFBE_Client_SideJoined, missionNamespace getVariable Format ["WFBE_%1ArtyRadarTYPE",WFBE_Client_SideJoinedText],_buildings] Call WFBE_CO_FNC_GetFactories;
		...
		_checks = ['COMMANDCENTERTYPE',_buildings,_ccr,WFBE_Client_SideJoined,player] Call WFBE_CO_FNC_BuildingInRange;
