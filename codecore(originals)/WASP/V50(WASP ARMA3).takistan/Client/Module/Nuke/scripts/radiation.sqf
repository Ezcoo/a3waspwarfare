...
_all_radius  = argt_nuclear_radius * 1.3;
_radius = _all_radius;
_v = _radius / (2 * argt_nuclear_half_life);
...
while {_radius > 1} do
{
  ...
  sleep (1 + random 1);
  if ( (time - _blast_time) > argt_nuclear_half_life ) then
  {
    _radius = _radius / 2;
    _blast_time = _blast_time + argt_nuclear_half_life;
    _v = _radius / (2 * argt_nuclear_half_life);
  };
};