...
if (isNil {missionNamespace getVariable Format ["WFBE_%1UAV",WFBE_Client_SideJoinedText]}) exitWith {};
if ((missionNamespace getVariable Format ["WFBE_%1UAV",WFBE_Client_SideJoinedText]) == "") exitWith {};

_buildings = (WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideStructures;
_checks = [WFBE_Client_SideJoined,missionNamespace getVariable Format ["WFBE_%1COMMANDCENTERTYPE",WFBE_Client_SideJoinedText],_buildings] Call WFBE_CO_FNC_GetFactories;
_closest = objNull;
if (count _checks > 0) then { _closest = [player,_checks] Call WFBE_CO_FNC_GetClosestEntity; };

if (isNull _closest) exitWith {};

_uav = createVehicle [missionNamespace getVariable Format ["WFBE_%1UAV",WFBE_Client_SideJoinedText],getPos _closest, [], 0, "FLY"];
playerUAV = _uav;
Call Compile Format ["_uav addEventHandler ['Killed',{[_this select 0,_this select 1,%1] Spawn WFBE_CO_FNC_OnUnitKilled}]",sideID];

[[[_uav,WFBE_Client_SideJoined], "Common\Init\Init_Unit.sqf"], "BIS_fnc_execVM", true, true] call BIS_fnc_MP;
removeAllWeapons _uav;

_group = createGroup [WFBE_Client_SideJoined, true];
_driver = [missionNamespace getVariable Format ["WFBE_%1SOLDIER",WFBE_Client_SideJoinedText],_group,getPos _uav,WFBE_Client_SideID] Call WFBE_CO_FNC_CreateUnit;
...
if (WFBE_Client_SideJoined == west) then {
	_gunner = [missionNamespace getVariable Format ["WFBE_%1SOLDIER",WFBE_Client_SideJoinedText],_group,getPos _uav,WFBE_Client_SideID] Call WFBE_CO_FNC_CreateUnit;
	_gunner MoveInGunner _uav;
	_built = _built + 1;
};
[WFBE_Client_SideJoinedText,'UnitsCreated',_built] Call WFBE_CO_FNC_UpdateStatistics;
[WFBE_Client_SideJoinedText,'VehiclesCreated',1] Call WFBE_CO_FNC_UpdateStatistics;

-12500 Call WFBE_CL_FNC_ChangePlayerFunds;

["uav",WFBE_Client_SideJoined,_uav,WFBE_Client_Team] remoteExecCall ["WFBE_SE_PVF_RequestSpecial",2];
