    ...
    if(WSW_newBuyRolerequest)then{
        ...
        switch (_status) do {
            ...
            case "success": {                
                ...
                WFBE_SK_V_Type = _roleDetails select 0;
                ...
                if(!WSW_isFirstRoleSelected)then {
                    ...
                    switch (WFBE_SK_V_Type) do {
                        case WSW_SNIPER: {_roleDefaultGear = missionNamespace getVariable Format["WFBE_%1_DefaultGearSpot", WFBE_Client_SideJoinedText];};
                        case WSW_SOLDIER: {_roleDefaultGear = missionNamespace getVariable Format["WFBE_%1_DefaultGearSoldier", WFBE_Client_SideJoinedText];};
                        case WSW_ENGINEER: {_roleDefaultGear = missionNamespace getVariable Format["WFBE_%1_DefaultGearEngineer", WFBE_Client_SideJoinedText];};
                        case WSW_SPECOPS: {_roleDefaultGear = missionNamespace getVariable Format["WFBE_%1_DefaultGearLock", WFBE_Client_SideJoinedText];};
                        case WSW_ARTY_OPERATOR: {_roleDefaultGear = missionNamespace getVariable Format["WFBE_%1_DefaultGearArtOperator", WFBE_Client_SideJoinedText];};
                        case WSW_UAV_OPERATOR: {_roleDefaultGear = missionNamespace getVariable Format["WFBE_%1_DefaultGearUAVOperator", WFBE_Client_SideJoinedText];};
                    };
