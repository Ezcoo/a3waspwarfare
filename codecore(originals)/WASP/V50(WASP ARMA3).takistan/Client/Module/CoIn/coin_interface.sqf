...
_bns = missionNamespace getVariable Format["WFBE_%1STRUCTURENAMES",WFBE_Client_SideJoinedText];
...
_buildingsType = missionNamespace getVariable Format["WFBE_%1STRUCTURES",WFBE_Client_SideJoinedText];
...
BIS_CONTROL_CAM_Handler = {
	...
	if (_mode == "keydown") then {
		...
		if(_key in _keyAutoWallConstructing)then{
			...
			(format ["Auto Wall Construction is: %1", _text]) Call WFBE_CL_FNC_GroupChatMessage;
			[_isAutoWallConstructingEnabled, player] remoteExecCall ["WFBE_SE_PVF_RequestAutoWallConstructinChange",2];
		};

		//--- Last Built Defense (Custom Action #1).
		if ((_key in (actionKeys "User15")) && count lastBuilt > 0) then {
			_deployed = true;
			if (WFBE_COIN_Root == "HQ") then {_deployed = (WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideHQDeployStatus};
			_currentCash = Call WFBE_CL_FNC_GetPlayerFunds;
		...
		if ((_key in (actionKeys "User17")) && !isNull(commanderTeam)) then {
			if(commanderTeam == WFBE_Client_Team) then {
				...
				if (isNil "_preview") then {//--- Proceed when there is no preview.
					...
					_defense_list = missionNamespace getVariable Format["WFBE_%1DEFENSENAMES",WFBE_Client_SideJoinedText];
					_defense_list pushBack "Land_HBarrier_large";
					...
					if (count _near > 0) then {
                      ...
                      if ((player distance _closest) > ((_logic getVariable "BIS_COIN_areasize") select 0) || (_closest getVariable 'side') != WFBE_Client_SideJoined) exitWith {};
						if (!isNil '_get' && isNil '_sold') then {
							_closest setVariable ['sold',true];
							_price = _get select QUERYUNITPRICE;
							round(_price/2.5) Call WFBE_CL_FNC_ChangePlayerFunds;
							_area = [getPos (_closest),((WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideLogic) getVariable "wfbe_basearea"] Call WFBE_CO_FNC_GetClosestEntity2;
							_get = _area getVariable 'avail';

							if (!isNull _area && _get < missionNamespace getVariable "WFBE_C_BASE_AV_STRUCTURES") then {
							_commanderTeam =(WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetCommanderTeam;
							_area setVariable [ "avail" ,_get +1];
							hintSilent parseText format ["Available Items : " +"<t color='#00FF00'>"+" %1"+"</t>", _area getVariable 'avail'];
							['Available', _area getVariable 'avail'] remoteExecCall ["WFBE_CL_FNC_HandleSpecial", leader commanderTeam];
							};
	...
	if (_mode == "mousemoving" || _mode == "mouseholding") then {
		...
		if (count _params > 0) then {
			...
			if (typename _funds == "ARRAY") then {
				_a = (WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideSupply;
				_b = Call WFBE_CL_FNC_GetPlayerFunds;
				_funds = [_a]+[_b];
			} else {
			...
			if (typeof _preview != _itemclass_preview) then {
				...
				if (_index == 0 && _hqDeployed) exitWith {
					_mhq = (WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideHQ;
					(_mhq) Spawn WFBE_CL_FNC_HandleHQAction;
					[WFBE_Client_SideJoined,_itemclass,[0,0,0],0] remoteExecCall ["WFBE_SE_PVF_RequestStructure",2];

					[missionNamespace getVariable "WFBE_C_BASE_COIN_AREA_HQ_UNDEPLOYED",false,MCoin] Call WFBE_CL_FNC_Init_Coin;

					_structuresCosts = missionNamespace getVariable Format["WFBE_%1STRUCTURECOSTS",WFBE_Client_SideJoinedText];
					[WFBE_Client_SideJoined,-(_structuresCosts select _index)] Call WFBE_CO_FNC_ChangeSideSupply;
				...
				if (_itemclass in _greenList && _index != -1) then {
					_distance = (missionNamespace getVariable Format ["WFBE_%1STRUCTUREDISTANCES",WFBE_Client_SideJoinedText]) select _index;
					_direction = (missionNamespace getVariable Format ["WFBE_%1STRUCTUREDIRECTIONS",WFBE_Client_SideJoinedText]) select _index;
					_npos = [getPos _preview,_distance,getDir _preview + _direction] Call WFBE_CO_FNC_GetPositionFrom;
			...
			if (!_new && !isNil "_preview" &&  ((BIS_CONTROL_CAM_LMB && 65536 in (actionKeys "DefaultAction")) || {_x in (actionKeys "DefaultAction")} count BIS_CONTROL_CAM_keys > 0) && _color == _colorGreen) then {
				...
				[_logic,_itemclass,_pos,_dir,_params] spawn {
					...
					//--- On Purchase.
					_structures = missionNamespace getVariable Format["WFBE_%1STRUCTURENAMES",WFBE_Client_SideJoinedText];
					_defenses = missionNamespace getVariable Format["WFBE_%1DEFENSENAMES",WFBE_Client_SideJoinedText];
					_costs = missionNamespace getVariable Format["WFBE_%1STRUCTURECOSTS",WFBE_Client_SideJoinedText];

					//--- Structures.
					_index = _structures find _itemclass;
					if (_index != -1) then {
						_price = _costs select _index;
						[WFBE_Client_SideJoined, -_price] Call WFBE_CO_FNC_ChangeSideSupply;
						...
						if (_index == 0) then {
							...
							[player,score player + (missionNamespace getVariable "WFBE_C_PLAYERS_COMMANDER_SCORE_BUILD")] remoteExecCall ["WFBE_SE_PVF_RequestChangeScore",2];
						};
					};
					...
					_deployed = (WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideHQDeployStatus;
					_structures = missionNamespace getVariable Format["WFBE_%1STRUCTURENAMES",WFBE_Client_SideJoinedText];
					_defenses = missionNamespace getVariable Format["WFBE_%1DEFENSENAMES",WFBE_Client_SideJoinedText];
					...
					if (_find != -1) then {
						...
						[WFBE_Client_SideJoined,_itemclass,_pos,_dir] remoteExecCall ["WFBE_SE_PVF_RequestStructure",2];
					};

					if (_itemclass in _defenses) then {
						[WFBE_Client_SideJoined,_itemclass,_pos,_dir,manningDefense] remoteExecCall ["WFBE_SE_PVF_RequestDefense",2];

						lastBuilt = _par;
						_area = [_pos,((WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideLogic) getVariable "wfbe_basearea"] Call WFBE_CO_FNC_GetClosestEntity2;
						_get = _area getVariable 'avail';
						if (!isNull _area && _get > 0) then {
							_commanderTeam =(WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetCommanderTeam;
							_area setVariable [ "avail" ,_get -1];
							hintSilent parseText format ["Available Items : " +"<t color='#00FF00'>"+" %1"+"</t>", _area getVariable 'avail'];
							[_area getVariable 'avail'] remoteExecCall ["WFBE_CL_FNC_Available",leader _commanderTeam];
						};
					};
				};
