...
_index = (missionNamespace getVariable Format ["WFBE_%1STRUCTURENAMES",WFBE_Client_SideJoinedText]) find _type;
if (_index != -1) then {
	...
	_factoryType = (missionNamespace getVariable Format ["WFBE_%1STRUCTURES",WFBE_Client_SideJoinedText]) select _index;
	...
	//_position = [getPos _building,_distance,getDir _building + _direction] Call WFBE_CO_FNC_GetPositionFrom;
	_position = _building modelToWorld [(sin _direction * _distance), (cos _direction * _distance), 0];
	_position set [2, .5];
	_longest = missionNamespace getVariable Format ["WFBE_LONGEST%1BUILDTIME",_factoryType];
};
...
if (_isMan) then {
	...
	_vehicle = [_unit, _position, sideID, _direction, _locked, nil, nil, nil, _unitdescription, _unitskin] Call WFBE_CO_FNC_CreateVehicle;
	
	WFBE_Client_Team reveal _vehicle;
	...
	//--- Salvage Truck.
	if (_unit in (missionNamespace getVariable Format['WFBE_%1SALVAGETRUCK',WFBE_Client_SideJoinedText])) then {[_vehicle] spawn WFBE_CL_FNC_Update_Salvage};

	//--- Are we dealing with an artillery unit.
	_isArtillery = [_unit,WFBE_Client_SideJoinedText] Call WFBE_CO_FNC_IsArtillery;
	if (_isArtillery != -1) then {[_vehicle,_isArtillery,WFBE_Client_SideJoinedText] Call WFBE_CO_FNC_EquipArtillery;};	

	/* Section: Creation */

	[WFBE_Client_SideJoinedText,'VehiclesCreated',1] Call WFBE_CO_FNC_UpdateStatistics;
...
if({(_vehicle isKindOf _x)} count ["Tank","Wheeled_APC"] !=0) then {_vehicle addeventhandler ['Engine',{_this execVM "Client\Module\Engines\Engine.sqf"}];
	...
	//--- Crew Management.
	_crew = missionNamespace getVariable Format ["WFBE_%1SOLDIER",WFBE_Client_SideJoinedText];
	if (_unit isKindOf "Tank") then {_crew = missionNamespace getVariable Format ["WFBE_%1CREW",WFBE_Client_SideJoinedText]};
	if (_unit isKindOf "Air") then {
		_crew = missionNamespace getVariable Format ["WFBE_%1PILOT",WFBE_Client_SideJoinedText];
	};
	...
	[WFBE_Client_SideJoinedText,'UnitsCreated',_cpt] Call WFBE_CO_FNC_UpdateStatistics;
};
