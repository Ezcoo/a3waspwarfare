...
_Upgrades = ((WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideUpgrades) select WFBE_UP_GEAR;

//_Upgrades		= if (side Player == west) then {Public_UpgradesBF} else {Public_UpgradesOF};	// Upgrades Data
//_upgrade_level = ((WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideUpgrades) select WFBE_UP_GEAR;
