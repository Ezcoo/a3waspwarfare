...
switch (missionNamespace getVariable "WFBE_C_RESPAWN_CAMPS_MODE") do {
	case 1: {
		/* Classic Respawn */
		_town = [_deathLoc] Call WFBE_CO_FNC_GetClosestLocation;
		if !(isNull _town) then {
			if (_town distance _deathLoc  < (missionNamespace getVariable "WFBE_C_RESPAWN_CAMPS_RANGE")) then {
				_camps = [_town,_side,true] Call WFBE_CO_FNC_GetFriendlyCamps;
				if (count _camps > 0) then {
					if (_respawnCampsRuleMode > 0) then {
						...
						_hostiles = [_closestCamp,_enemySide,_respawnMinRange] Call WFBE_CO_FNC_GetHostilesInArea;
	...
	case 2: {
		...
		_nearestCamps = _deathLoc nearEntities [WFBE_Logic_Camp, (missionNamespace getVariable "WFBE_C_RESPAWN_CAMPS_RANGE")];
		{
			if !(isNil {_x getVariable 'sideID'}) then {
				if ((_side Call WFBE_CO_FNC_GetSideID) == (_x getVariable 'sideID') && alive(_x getVariable "wfbe_camp_bunker")) then {
					if (_respawnCampsRuleMode > 0) then {
						if (_deathLoc distance _x < _respawnMinRange) then {
							...
							_hostiles = [_x,_enemySide,_respawnMinRange] Call WFBE_CO_FNC_GetHostilesInArea;
	...
	case 3: {
		...
		_nearestCamps = _deathLoc nearEntities [WFBE_Logic_Camp, (missionNamespace getVariable "WFBE_C_RESPAWN_CAMPS_RANGE")];
		{
			if !(isNil {_x getVariable 'sideID'}) then {
				...
				if ((_side Call WFBE_CO_FNC_GetSideID) == _townSID && (_x getVariable 'sideID') == _townSID && alive(_x getVariable "wfbe_camp_bunker")) then {
					if (_respawnCampsRuleMode > 0) then {
						if (_deathLoc distance _x < _respawnMinRange) then {
							if (WFBE_ISTHREEWAY) then  {
								_enemySide = [west, east, resistance] - [_side];
