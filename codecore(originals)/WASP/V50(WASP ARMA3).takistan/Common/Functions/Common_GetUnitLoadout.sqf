...
_uniform_items = (uniformItems _target) call WFBE_CO_FNC_ArrayToLower;
_vest = toLower(vest _target);
_vest_items = (vestItems _target) call WFBE_CO_FNC_ArrayToLower;
_backpack = toLower(backpack _target);
_backpack_items = (backpackItems _target) call WFBE_CO_FNC_ArrayToLower;

//--- Weapons
_primary = toLower(primaryWeapon _target);
_primary_accessories = (primaryWeaponItems _target) call WFBE_CO_FNC_ArrayToLower;
_secondary = toLower(secondaryWeapon _target);
_secondary_accessories = (secondaryWeaponItems _target) call WFBE_CO_FNC_ArrayToLower;
_handgun = toLower(handgunWeapon _target);
_handgun_accessories = (handgunItems _target) call WFBE_CO_FNC_ArrayToLower;

//--- Currently loaded magazines
_primary_current_magazine = (primaryWeaponMagazine _target) call WFBE_CO_FNC_ArrayToLower;
_secondary_current_magazine = (secondaryWeaponMagazine _target) call WFBE_CO_FNC_ArrayToLower;
_handgun_current_magazine = (handgunMagazine _target) call WFBE_CO_FNC_ArrayToLower;
...
//--- Items
_allitems = ((assignedItems _target) call WFBE_CO_FNC_ArrayToLower) - [_headgear, _goggles];
_items = [["", ""], ["", "", "", "", ""]];

{
	...
	if (_slot select 0 == -1) then { //--- The simulation couldn't be determined, try to get the subtype maybe?
		if (getNumber(configFile >> 'CfgWeapons' >> _x >> 'ItemInfo' >> 'type') isEqualTo WFBE_SUBTYPE_UAVTERMINAL) then {_slot = [1,1]};
...
_items = [(_items select 0) call WFBE_CO_FNC_ArrayToLower, (_items select 1) call WFBE_CO_FNC_ArrayToLower];
