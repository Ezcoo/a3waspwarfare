...
_found = [];
{
	_structure_type = if (isNil{_x getVariable "wfbe_structure_type"}) then {""} else {_x getVariable "wfbe_structure_type"};
...
if (count _found > 0 && typeName _sort in ["OBJECT","POSITION"]) then { 
	...
	_found = [_sort, _found] call WFBE_CO_FNC_SortByDistance;
};
