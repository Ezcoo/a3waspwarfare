...

{
	if (_x isKindOf 'Man') then {
		...
		_type = if (_vehicle isKindOf 'Man') then {
			missionNamespace getVariable Format ['WFBE_%1SOLDIER',_side]
		} else {
		...
		if (_vehicle emptyPositions 'driver' > 0) then {
			_driver = [_type,_team,_position,_sideID] Call WFBE_CO_FNC_CreateUnit;
			_driver moveInDriver _vehicle;
			_crews pushBack _driver
		};
		if (_vehicle emptyPositions 'gunner' > 0) then {
			_gunner = [_type,_team,_position,_sideID] Call WFBE_CO_FNC_CreateUnit;
			_gunner moveInGunner _vehicle;
			_crews pushBack _gunner
		};
		if (_vehicle emptyPositions 'commander' > 0) then {
			_commander = [_type,_team,_position,_sideID] Call WFBE_CO_FNC_CreateUnit;
