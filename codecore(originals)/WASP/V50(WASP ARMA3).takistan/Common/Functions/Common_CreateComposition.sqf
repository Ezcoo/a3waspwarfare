...
_objs   = missionNamespace getVariable "WFBE_NEURODEF_MEDIUM_ROADBLOCK";
_posX = _pos select 0;
_posY = _pos select 1;
_defences = [];
_fortifications = [];
...
for "_i" from 0 to ((count _objs) - 1) do {
	...
	if(_type == (missionNamespace getVariable "WFBE_C_CAMP"))then{
		_newObj = _campObject;
	} else {
    ...
    if(_type != (missionNamespace getVariable "WFBE_C_CAMP"))then{
        if(_type in WFBE_C_STATIC_DEFENCE_FOR_COMPOSITIONS)then{
            _newObj setVariable ['wfbe_defense_kind', _static_kind];
            _newObj setVariable ["wfbe_defense", _newObj];
...
_town_defences_composition = _town getvariable "wfbe_town_defenses_composition";
if(isNil "_town_defences_composition")then{
    _town setVariable ["wfbe_town_defenses_composition", _defences];
}else{
    for "_i" from 0 to ((count _defences) - 1) do
    {
        _town_defences_composition pushBack (_defences select _i);
    };
    _town setVariable ["wfbe_town_defenses_composition", _town_defences_composition];
};

_town_fortifications_composition = _town getvariable "wfbe_town_fortifications_composition";
if(isNil "_town_fortifications_composition")then{
    _town setVariable ["wfbe_town_fortifications_composition", _fortifications];
}else{
    for "_i" from 0 to ((count _fortifications) - 1) do
    {
        _town_fortifications_composition pushBack (_fortifications select _i);
    };
    _town setVariable ["wfbe_town_fortifications_composition", _town_fortifications_composition];
};