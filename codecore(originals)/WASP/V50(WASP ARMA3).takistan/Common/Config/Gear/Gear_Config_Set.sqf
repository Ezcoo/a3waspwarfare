	...
	if (_config_type != "nil") then { //--- Make sure that it exists
		if (isNil {missionNamespace getVariable _item}) then {
			switch (_config_type) do { //--- Each item is handled differently
				case "CfgGlasses": {
					missionNamespace setVariable [format["wfbe_%1", _item], [[_upgrades select _i, _prices select _i, ""], "Goggles", _config_type]];
					if (_pushtogear) then {_gear_glasses pushBack _item};
					_cpt = _cpt + 1;
				};
				case "CfgMagazines": {
					missionNamespace setVariable [format["wfbe_%1", _item], [[_upgrades select _i, _prices select _i, ""], "Magazines", _config_type]];
				...
				case "CfgVehicles": {
					if (getNumber(configFile >> _config_type >> _item >> 'isbackpack') == 1) then { //--- Make sure that this is a backpack
						missionNamespace setVariable [format["wfbe_%1", _item], [[_upgrades select _i, _prices select _i, ""], "Backpack", _config_type]];
				...
				case "CfgWeapons": {
					...
					missionNamespace setVariable [format["wfbe_%1", _item], [[_upgrades select _i, _prices select _i, ""], _item_type, _config_type]];
					_cpt = _cpt + 1;
				};
			};
		};
...
} forEach [["wfbe_gear_list_primary", _gear_primary],["wfbe_gear_list_secondary", _gear_secondary],["wfbe_gear_list_pistol", _gear_pistol],["wfbe_gear_list_magazines", _gear_magazines],
["wfbe_gear_list_accessories", _gear_accessories],["wfbe_gear_list_misc", _gear_misc],["wfbe_gear_list_special", _gear_special],["wfbe_gear_list_uniforms", _gear_uniforms],
["wfbe_gear_list_vests", _gear_vests],["wfbe_gear_list_backpacks", _gear_backpacks],["wfbe_gear_list_headgear", _gear_headgear],["wfbe_gear_list_glasses", _gear_glasses],
["wfbe_gear_list_explosives", _gear_explosives]];