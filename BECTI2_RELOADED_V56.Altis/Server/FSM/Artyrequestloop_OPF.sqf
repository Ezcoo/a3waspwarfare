...
_enemyOPF pushBackUnique _temp;
};} forEach _opf;
//systemChat str _enemyOPF;


//GET ARTYUNITS
_artyOPF=[];
{if ((typeOf vehicle _x) in _artyvehicle) then {_artyOPF pushBack _x};} forEach _opf;
//systemChat str _artyOPF;

//DO ARTYREQUEST

if ((count _enemyOPF) > 0) then{
...
{if ((_target distance2D _x) < 4000) then {_artyINrange pushBack _x};} forEach _artyOPF;

//systemChat "in Range=";
//systemChat str _artyINrange;

_minus_short_range_Units=[];
{if (((_target distance2D _x) > 2000)&&(_x in _artyvehicle_short)) then {_minus_short_range_Units pushBack _x};} forEach _artyOPF;
...
{if (_x in _artyvehicle_short) then {_shortrange_in_range pushBack _x};} forEach _artyOPF;
...
//systemChat "OPF FEUERT nur die kurzen";
_enemyOPF= [];
_artyINrange=[];
_shortrange_in_range=[];
};

_UpArtytime = ((east) Call EZC_fnc_Functions_Common_GetSideUpgrades) select cti_UP_ARTYTIMEOUT;
