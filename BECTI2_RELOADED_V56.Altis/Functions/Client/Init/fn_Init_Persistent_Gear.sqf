...
if (_templates isEqualType []) then { //--- The variable itself is an array
	["INFORMATION", "FILE: Client\Init\Init_Persistent_Gear.sqf", format["The client has [%1] templates in it's profile", count _templates]] call EZC_fnc_Functions_Common_LogContent;

	{
		// [_label, _picture, _cost, _x]

		private _err_reason = "";
		["INFORMATION", "FILE: Client\Init\Init_Persistent_Gear.sqf", format["Attempting to load persistent template number [%1]", _forEachIndex]] call EZC_fnc_Functions_Common_LogContent;

		if (_x isEqualType []) then { //--- Each items are arrays >> [_label, _picture, _cost, _x, upgrade]
			private _gear = _x select 3;
			if ((_x select 0) isEqualType "" && (_x select 1) isEqualType "" && _gear isEqualType []) then { //--- The label is a string, the picture is a string and the template is an array. Cost and upgrade are re-calculated at the end to prevent third party modification
				if (count _gear isEqualTo 4) then { //--- Make sure that we have the sections (weapons/container+mags/equipment/equipment2)
					private _flag_load = true;

					//--- #1 Now the party begin! first we check the weapons (primary/secondary/handgun)
					private _gear_sub = _gear select 0;
					if (_gear_sub isEqualType []) then { //--- The weapons items are stored in an array >> [["arifle_mxc_f",["","acc_pointer_ir","optic_Aco"],["30rnd_65x39_caseless_mag"]],["launch_nlaw_f",[],["nlaw_f"]],["",[],[]]]
						{
							...
							if (_x isEqualType []) then {
								if (count _x isEqualTo 3) then { //--- Make sure that we have the weapon/accessories/current magazine
									...
									if (_weapon isEqualType "" && _accessories isEqualType [] && _magazine isEqualType []) then { //--- The data format is valid
										...
										{
											if (_x isEqualType "") then { //--- The accessory is a string
										...
										if (count _magazine isEqualTo 2) then {
											_magazine = _magazine select 1;
											if (!isClass (configFile >> "CfgMagazines" >> _magazine) || !(_magazine in _side_gear)) exitWith {_flag_load = false; _err_reason = format["Magazine [%1] is either not a valid CfgMagazines class or does not belong to the player side", _magazine]};
										};
					...
					if (_flag_load) then {
						...
						if (_gear_sub isEqualType []) then { //--- The weapons items are stored in an array >> [["u_b_combatuniform_mcam",[]],["v_platecarrier1_rgr",[]],["b_assaultpack_mcamo",["firstaidkit","firstaidkit","handgr...
										...
										if (count _x isEqualTo 2) then { //--- Each array need to have the container and the items
											if ((_x select 0) isEqualType "" && (_x select 1) isEqualType []) then { //--- The container is a string, the items are an array
												...
												if !(_container isEqualTo "") then { //--- If the container is empty, we don't bother any further
													...
													{
														if !(_x isEqualType "") exitWith {_flag_load = false; _err_reason = format["Container [%1] item [%2] is not a string", _container, _x]};
														private _config_type = switch (true) do { //--- Determine the kind of item that we're dealing with
					...
					if (_flag_load) then {
						...
						if (_gear_sub isEqualType []) then {
							if (count _gear_sub isEqualTo 2) then {
								private _helm = _gear_sub select 0;
								private _goggles = _gear_sub select 1;
						...
						if (_flag_load) then {
							...
							if (_gear_sub isEqualType []) then {
								if (count _gear_sub isEqualTo 2) then {
									//--- Check nvg/binocs
									if ((_gear_sub select 0) isEqualType []) then {
										if (count(_gear_sub select 0) isEqualTo 2) then {
											{
												if !(_x isEqualType "") exitWith {_flag_load = false; _err_reason = format["Item [%1] is not a string", _x]};
												...
												} forEach [((_gear_sub select 0) select 0)];
										} else {
											_flag_load = false;
									...
									//--- Check items
									if ((_gear_sub select 1) isEqualType []) then {
										if (count(_gear_sub select 1) isEqualTo 5) then {
											{
												if !(_x isEqualType "") exitWith {_flag_load = false; _err_reason = format["Item (Binoc/NVG) [%1] is not a string", _x]};
												if !(_x isEqualTo "") then {
							...
							if (_flag_load) then {
								...
								if (CTI_LogLevel >= CTI_Log_Information) then {
									["INFORMATION", "FILE: Client\Init\Init_Persistent_Gear.sqf", format["A persistent template has been loaded with the label [%1], cost of [%2], upgrade level of [%3]", _x select 0, _cost, _upgrade_max]] call EZC_fnc_Functions_Common_LogContent;
								};
		...
		if !(_err_reason isEqualTo "") then {
			["ERROR", "FILE: Client\Init\Init_Persistent_Gear.sqf", format["The persistent template number [%1] could not be loaded due to the following error [%2]", _forEachIndex, _err_reason]] call EZC_fnc_Functions_Common_LogContent;
		};
	} forEach _templates;
} else {
	["ERROR", "FILE: Client\Init\Init_Persistent_Gear.sqf", "The persistent gear variable type does not match the expected ARRAY type, the persistent gear will not be loaded."] call EZC_fnc_Functions_Common_LogContent;
};
