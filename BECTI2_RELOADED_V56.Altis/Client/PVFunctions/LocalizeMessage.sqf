...
_txt = "";
switch (_localize) do {
	case "BuildingTeamkill": {_txt = Format [Localize "STR_WF_CHAT_Teamkill_Building",_this select 1, _this select 2, [_this select 3, 'displayName'] Call EZC_fnc_Functions_Common_GetConfigInfo]};
	...
	case "StructureSold": {_txt = Format [Localize "STR_WF_CHAT_Structure_Sold",([_this select 1,'displayName'] Call EZC_fnc_Functions_Common_GetConfigInfo)]};
	case "StructureSell": {_txt = Format [Localize "STR_WF_CHAT_Structure_Sell",([_this select 1,'displayName'] Call EZC_fnc_Functions_Common_GetConfigInfo),_this select 2]};
	...
	case "StructureTK": {_txt = Format [Localize "STR_WF_CHAT_SatchelTK",_this select 1, _this select 2, [_this select 3, 'displayName'] Call EZC_fnc_Functions_Common_GetConfigInfo, _this select 4]};
    ...
    case "HeadHunterReceiveBounty":
    {
        ...
        {
            _txt = format [localize "STR_WF_HeadHunterReceiveBounty", _bounty, ([_structure_kind, "displayName"] call EZC_fnc_Functions_Common_GetConfigInfo)];
        ...
        else
        {
            ...
            {
                _txt = format [localize "STR_WF_HeadHunterReceiveBountyFriendly", _killer_name, _bounty, ([_structure_kind, "displayName"] call EZC_fnc_Functions_Common_GetConfigInfo)];
            }
            else
            {
                _txt = format [localize "STR_WF_HeadHunterReceiveBountyEnemy", _killer_name, _bounty, ([_structure_kind, "displayName"] call EZC_fnc_Functions_Common_GetConfigInfo)];
            };
            _commandChat = true;
        };
    };
    case "HeadHunterReceiveBountyInSupplies":{
        ...
        if (_side_killer != _structure_side) then{
            if(_supplies_bounty > 0)then{
                _txt = format [localize "STR_WF_HeadHunterReceiveSuppliesEnemy", _side_killer, _supplies_bounty, ([_structure_kind, "displayName"] call EZC_fnc_Functions_Common_GetConfigInfo)];
                _commandChat = true;
            };
        }
    ...
    case "BuildingKilledByError":
    {
        ...
        if ((side player) == _structure_side) then
        {
            _txt = format [localize "STR_WF_BuildingKilledByErrorFriendly", ([_structure_kind, "displayName"] call EZC_fnc_Functions_Common_GetConfigInfo)];
        }
        else
        {
            _txt = format [localize "STR_WF_BuildingKilledByErrorEnemy", ([_structure_kind, "displayName"] call EZC_fnc_Functions_Common_GetConfigInfo)];
        };
        _commandChat = true;
    };
};
