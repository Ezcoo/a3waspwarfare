...
[_pos, _side] Spawn {
	...
	if(_side == side player)then{
		...
		_artyRadarInRange = if (isNull (['ArtyRadarTYPE',_buildings,_radar_range,cti_Client_SideJoined,player] Call EZC_fnc_Functions_Common_BuildingInRange)) then {false} else {true};
