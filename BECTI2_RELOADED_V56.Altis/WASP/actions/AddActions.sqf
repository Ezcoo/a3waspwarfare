...
//BuyHQ = leader(group player) addAction ["<t color='#FF0000'>"+ "RECOVER HQ" + "  " + str (missionNameSpace getVariable 'cti_C_BASE_HQ_REPAIR_PRICE_CASH') +"$" +"</t>", "Client\Action\Action_RepairMHQDepot.sqf", [], 1, false, true, "", "(!(alive ((cti_Client_SideJoined) Call EZC_fnc_Functions_Common_GetSideHQ)))&&(leader  (cti_Client_SideJoined call EZC_fnc_Functions_Common_GetCommanderTeam) == leader (vehicle player))"];		
	BuyHQ = leader(group player) addAction ["<t color='#FF0000'>"+ "RECOVER HQ" + "  " + str (missionNameSpace getVariable 'cti_C_BASE_HQ_REPAIR_PRICE_CASH') +"$" +"</t>", "Client\Action\Action_RepairMHQDepot.sqf", [], 1, false, true, "", "(!(alive ((cti_Client_SideJoined) Call EZC_fnc_Functions_Common_GetSideHQ)))&&(leader  (cti_Client_SideJoined call EZC_fnc_Functions_Common_GetCommanderTeam) == leader (vehicle player))&&(typeOf cursorTarget in ['land_bunker_garage','cti_C_DEPOT'])&&(cursorTarget distance player < 100)"];		
