...
if !(isNil '_get') then { //--- Retrieve JIP Information if there's any.
	...
	if (_skip == 0) then {
		...
		if(_players_difference > 2) then {
			...
			['Teamstack',_name,_uid,_side] remoteExecCall ["WFBE_CL_FNC_LocalizeMessage", _player];
		...
		if (_sideOrigin != _side) then { //--- The joined side differs from the original one.
			...
			['Teamswap',_name,_uid,_sideOrigin,_side] remoteExecCall ["WFBE_CL_FNC_LocalizeMessage"];
...
['join-answer', _canJoin] remoteExecCall ["WFBE_CL_FNC_HandleSpecial", _player];