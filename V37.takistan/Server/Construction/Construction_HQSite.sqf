...
if (!_deployed) then {
	...
	[_side,"Deployed", ["Base", _site]] Spawn WFBE_SE_FNC_SideMessage;
	...
	_site addEventHandler ['handleDamage',{[_this select 0,_this select 2,_this select 3, _this select 4] Call WFBE_SE_FNC_BuildingHandleDamages}];
	...
	if ((missionNamespace getVariable "WFBE_C_BASE_AREA") > 0) then {
		...
		if (_update) then {
			...
			[_logic, _position,_side,_logik,_areas] remoteExecCall ["WFBE_CL_FNC_RequestBaseArea",_side];
		};
	};
	...
	_MHQ = [_HQName, [1,1,1], _sideID, _direction, true, false] Call WFBE_CO_FNC_CreateVehicle;
	...
	if (isMultiplayer) then {
		...
		["set-hq-lock-unlock-actions", _mhq] remoteExecCall ["WFBE_CL_FNC_HandleSpecial", _side];
	};
