...
_range_detect_active = missionNamespace getVariable "WFBE_C_TOWNS_AI_SPAWN_RANGE";
...
for "_k" from 0 to ((count towns) - 1) step 1 do
{
	...
	_town setVariable ['wfbe_town_active_vehicles',[]];
		...
		if(_sideID != WFBE_C_UNKNOWN_ID) then {
            ...
            if(_enemies > 0)then{
                ...
                if(!(_town getVariable "wfbe_active")) then {
                    ...
                    if (missionNamespace getVariable Format ["WFBE_%1_PRESENT",_side]) then {[_side,"HostilesDetectedNear",_town] Spawn WFBE_SE_FNC_SideMessage};
                    ...
                    for '_i' from 0 to count(_groups)-1 do {
                        ...
                        if (count _camps > 0 && random 100 > 25) then {
                            ...
                            _position = ([getPosATL _camp, _spawn_radius] call WFBE_CO_FNC_GetSafePlace);
                        } else {
                            _position = ([getPosATL _town, _spawn_radius] call WFBE_CO_FNC_GetSafePlace);
                        };
                        _positions pushBack _position;
                        _spawn_radius = _spawn_radius + 15;
                    };

                    [_town, _side, "spawn"] spawn WFBE_SE_FNC_OperateTownDefensesUnits;

                    if (count(missionNamespace getVariable "WFBE_HEADLESSCLIENTS_ID") > 0) then {
                        [_town, _side, _groups, _positions] spawn WFBE_SE_FNC_DelegateAITownHeadless
                    }else {
                        [_town, _side, _groups, _positions] spawn WFBE_CO_FNC_CreateTownUnits;
                    };
                    //// end of creating
                };
            };
				    ...
				    if(time - (_town getVariable "wfbe_inactivity") > _unitsInactiveMax) then {
                        ...
                        if!(isNil "headlessClients") then {
                            if(count headlessClients > 0)then{
                                ...
                                [_town getVariable "name"] remoteExecCall ["WFBE_CL_FNC_RemoveTownAI", _hc];
                            };
                        } else {
                            //--- Teams Units.
                            {
