...
if(count _alives == 0) then{
	{
		...
		if !(isNull _buyFrom) then {
			_IDS = [];
			[_IDS,_buyFrom,_x,_side,_inf_group] call WFBE_SE_FNC_Server_ResBuyUnit;
		};
	} forEach _template;
};
...
while{!_end}do{
	...
	if(_action_type == "moveto")then{
		...
		_near = [_barrack, _buildings] Call WFBE_CO_FNC_SortByDistance;
		_target = _near select 0;
		[_inf_group, true, [[_target, 'SAD', 100, 60, "", []]]] Call WFBE_SE_FNC_AIWPAdd;
	}else{
		if (_orderComplete) then {
			_orderComplete = false;
			_destination = [leader _inf_group, [_barrack]] Call WFBE_CO_FNC_GetClosestEntity;
			[_inf_group,_destination,400] Call WFBE_SE_FNC_AIPatrol;
		};
	};
	sleep 180;
};
