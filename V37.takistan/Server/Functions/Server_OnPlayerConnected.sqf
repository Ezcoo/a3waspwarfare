	...
	if !(isNil '_x') then {
		if (_x isKindOf "Man") then {
			...
			if((_teams pushBackUnique _group) != -1)then{
				if (isNil {_group getVariable "wfbe_funds"}) then {_group setVariable ["wfbe_funds", missionNamespace getVariable Format ["WFBE_C_ECONOMY_FUNDS_START_%1", _sideJoined], true]};
				_group setVariable ["wfbe_side", _sideJoined];
				...
				["INITIALIZATION", Format["Init_Server.sqf: [%1] Team [%2] was initialized.", _sideJoined, _group]] Call WFBE_CO_FNC_LogContent;
			};
		};
	};
...
if (_sideOrigin != _sideJoined) then {
	...
	if(_funds <= 0)then{
		_funds = missionNamespace getVariable Format ["WFBE_C_ECONOMY_FUNDS_START_%1", side (leader _team)];
	};
};
