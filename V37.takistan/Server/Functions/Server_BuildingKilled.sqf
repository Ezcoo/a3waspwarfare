...
if ((!isNull _killer) && (isPlayer _killer)) then{
    if (_teamkill) then{
		["BuildingTeamkill", name _killer, _killer_uid, _type] remoteExecCall ["WFBE_CL_FNC_LocalizeMessage", _side];
    }else{
        _supplies = 0;
         ...
         if(typeof _structure == "Gue_WarfareBBarracks")then{
            _bounty = 3000;
            _supplies = 500;
            ["HeadHunterReceiveBountyInSupplies", _side_killer, _type, _supplies, _side] remoteExecCall ["WFBE_CL_FNC_LocalizeMessage", _side_killer];
            ["HeadHunterReceiveBounty", (name _killer), _bounty, _type, _side] remoteExecCall ["WFBE_CL_FNC_LocalizeMessage", _side_killer];

            [_side_killer, _supplies] Call WFBE_CO_FNC_ChangeSideSupply;
         } else{
            ["HeadHunterReceiveBounty", (name _killer), _bounty, _type, _side] remoteExecCall ["WFBE_CL_FNC_LocalizeMessage", _side_killer];
         };

        if(_side == resistance)then{
            [_side, _structure] spawn WFBE_CO_FNC_CleanResBaseArea;
        }
	};
};
