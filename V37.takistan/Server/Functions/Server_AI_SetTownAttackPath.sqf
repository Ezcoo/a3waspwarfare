...
if (_wp_origin distance _wp_dest > _distance_node) then {
	...
	_wp_sel pushBack [([_select, 20, 100] Call WFBE_CO_FNC_GetRandomPosition), 'MOVE', 40, 20, [], [], ["AWARE","","WEDGE","NORMAL"]];
	...
	for '_i' from 0 to _nodes-1 do {
		...
		if !(_a_safe) exitWith {};
		if (_a_safe) then {_wp_sel pushBack [([_select, 20, 100] Call WFBE_CO_FNC_GetRandomPosition), 'MOVE', 60, 30, [], [], []]};
	};
...
if (random 100 > 50) then {
	...
	{
		_wp_sel pushBack [([_x, 10, 35] Call WFBE_CO_FNC_GetRandomPosition), 'SAD', 40, 20, [], [], _behaviour];
...
//--- Depot SAD.
_wp_sel pushBack [([_wp_dest, 10, 150] Call WFBE_CO_FNC_GetRandomPosition), 'SAD', 35, 25, [], [], ["AWARE","","FILE","NORMAL"]];
_wp_sel pushBack [([_wp_dest, 5, 25] Call WFBE_CO_FNC_GetRandomPosition), 'MOVE', 35, 25, [], [], ["AWARE","","FILE","NORMAL"]];
