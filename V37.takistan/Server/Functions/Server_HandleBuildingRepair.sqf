...
_redu = if (_type isKindOf "Warfare_HQ_base_unfolded") then {5} else {missionNamespace getVariable "WFBE_C_STRUCTURES_DAMAGES_REDUCTION"};
			...
			if ((_current select _index) < _limit) then {			
				...
				if !(isNull _site) then {
					...
					Call Compile Format ["_site AddEventHandler ['killed',{[_this select 0,_this select 1,'%1'] Spawn WFBE_SE_FNC_BuildingKilled}];",_type];
						[_side, -round((_buildingsCosts select _index)/2)] Call WFBE_CO_FNC_ChangeSideSupply;
