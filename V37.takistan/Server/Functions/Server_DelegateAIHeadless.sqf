...
if!(isNil "headlessClients") then {
	if(count headlessClients > 0)then{
		...
		if(isNil '_special')then{
		    ['delegate-ai', _side, _unitType, _position, _team, _dir] remoteExecCall ["WFBE_CL_FNC_HandleSpecial", _hc];
		} else {
		    ['delegate-ai', _side, _unitType, _position, _team, _dir, _special] remoteExecCall ["WFBE_CL_FNC_HandleSpecial", _hc];
		};
	};
};
