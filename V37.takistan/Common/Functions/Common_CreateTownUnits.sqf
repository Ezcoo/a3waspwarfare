...
for '_i' from 0 to count(_groups)-1 do {
	...
	["INFORMATION", Format["Common_CreateTownUnits.sqf: Town [%1] [%2] will create a team template %3 at %4", _town, _team, _groups select _i,_position]] Call WFBE_CO_FNC_LogContent;

	_retVal = [_groups select _i, _position, _side, _lock, _team, true, 90, _town] call WFBE_CO_FNC_CreateTeam;
    ...
    if(count _vehicles > 0) then {
        [_town, _team, _sideID] spawn WFBE_CO_FNC_SetPatrol;
    }else{
        /*_isPatrolSet = false;

        if(_c != (count _camps)-1)then{
            _camp = _camps select _c;
            _position = ([getPosATL _camp, 10] call WFBE_CO_FNC_GetSafePlace);
            diag_log format ["leader _team: %1", leader _team];
            (leader _team) setPosATL _position;
            [leader _team, _town_upsmon_marker, "COMBAT", "LINE", "LIMITED", "AMBUSH", "NOWP", "NOVEH2", "NOFOLLOW"] spawn WFBE_CO_FNC_SetUpsPatrol;
            _isPatrolSet = true;
	...
	//[_town, _team, _sideID] spawn WFBE_CO_FNC_SetPatrol;
	//[_team, 175, _position] spawn WFBE_CO_FNC_RevealArea;
...
if (_builtveh > 0) then {[str _side,'VehiclesCreated',_builtveh] call WFBE_CO_FNC_UpdateStatistics};
...
_town setVariable ['wfbe_active_vehicles', (_town getVariable 'wfbe_active_vehicles') + _town_vehicles];
_town setVariable ['wfbe_town_teams', (_town getVariable 'wfbe_town_teams') + _town_teams];