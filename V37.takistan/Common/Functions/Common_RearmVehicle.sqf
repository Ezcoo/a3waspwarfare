...
if ((typeOf _vehicle) in WFBE_C_UNITS_TO_BALANCE) then {	
...
{	
	if((_x select 0) == (typeOf _vehicle)) exitWith {		
		_vehicle setVehicleAmmoDef 1;
		[_vehicle, _x select 1] call WFBE_CO_FNC_Requip_AIR_VEH;
	};			
} forEach WFBE_C_AIR_VEHICLE_TO_REQUIP;
...
if(!(isNil { _vehicle getVariable "_pylons" } )) then {
	if(count (_vehicle getVariable "_pylons") > 0) then {			
		[_vehicle, _vehicle getVariable "_pylons"] call WFBE_CO_FNC_Requip_AIR_VEH;
	};
};

//--- Are we dealing with an artillery unit ?.
_isArtillery = [_type, _side] Call WFBE_CO_FNC_IsArtillery;
if (_isArtillery != -1) then {[_vehicle, _isArtillery, _side] Call WFBE_CO_FNC_EquipArtillery};
