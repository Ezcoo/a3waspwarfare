...
_killed_side = (_this select 2) Call WFBE_CO_FNC_GetSideFromID;
...
if (isServer) then {_killed setVariable ["wfbe_trashed", true];	_killed Spawn WFBE_CO_FNC_TrashObject};
if (_killed_isplayer) then {_killed setVariable ["wfbe_trashed", true];	_killed Spawn WFBE_CO_FNC_TrashObject};


if (_killed_side in WFBE_PRESENTSIDES) then { //--- Update the statistics if needed.
	if (_killed_isman) then {[str _killed_side,'Casualties',1] Call WFBE_CO_FNC_UpdateStatistics} else {[str _killed_side,'VehiclesLost',1] Call WFBE_CO_FNC_UpdateStatistics};
};
...
if (!isNil '_get' && _killer_iswfteam) then { //--- Make sure that type killed type is defined in the core files and that the killer is a WF team.
	if (_killer_side != _killed_side) then { //--- Normal kill.
		if (isPlayer (leader _killer_group)) then { //--- The team is lead by a player.
			...
			if !(_killer_isplayer) then { //--- An AI is the killer.
				...
				if (isServer) then {
					...
					[leader _killer_group, (score leader _killer_group) + _points] call WFBE_SE_PVF_RequestChangeScore;


				} else {
					[leader _killer_group, (score leader _killer_group) + _points] remoteExecCall ["WFBE_SE_PVF_RequestChangeScore",2];
				};
			};

			//--- Award the bounty if needed.
			if (_killed_isplayer && _killer_isplayer) then {
				[_killed] remoteExecCall ["WFBE_CL_FNC_AwardBountyPlayer", _killer];
				
			};

			[_killed_type, false, _killer_award] remoteExecCall ["WFBE_CL_FNC_AwardBounty", leader _killer_group];
			if (vehicle _killed != _killed && alive _killed) then { //--- Kill assist (players in the same vehicle).
				{
					if (alive _x && isPlayer _x) then {
						[_objectType, true] remoteExecCall ["WFBE_CL_FNC_AwardBounty", getPlayerUID(_x)];
					}
		...
		//--- Only applies to player groups.
			['Teamkill'] remoteExecCall ["WFBE_CL_FNC_LocalizeMessage", _killer];
		};
	};
};
