...
_vehicle call WFBE_CO_FNC_BalanceInit;
_vehicle call WFBE_CO_FNC_Change_Vehicle_Texture;
...
for "_x" from 0 to ((count WFBE_C_AIR_VEHICLE_TO_REQUIP) - 1) do
{	
	_currentElement = WFBE_C_AIR_VEHICLE_TO_REQUIP select _x;
	if ((typeOf _vehicle) in _currentElement || _description in _currentElement) exitWith {		
		[_vehicle, _currentElement select 1] call WFBE_CO_FNC_Requip_AIR_VEH;
	};	
};
...
if(typeOf _vehicle in WFBE_C_COMBAT_JETS_WITH_BOMBS) then {
	_vehicle addeventhandler ['Fired',{_this spawn WFBE_CO_FNC_HandleBombs}];
};
