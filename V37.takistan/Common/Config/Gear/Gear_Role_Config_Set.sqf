	...
	if (_config_type != "nil") then { //--- Make sure that it exists
		if (isNil {missionNamespace getVariable _item}) then {
			switch (_config_type) do { //--- Each item is handled differently
				case "CfgGlasses": {
					missionNamespace setVariable [format["wfbe_%1", _item], [[_upgrades select _i, _prices select _i, _roleName], "Goggles", _config_type]];
					if (_pushtogear) then {_gear_glasses pushBack _item};
					_cpt = _cpt + 1;
				};
				case "CfgMagazines": {
					missionNamespace setVariable [format["wfbe_%1", _item], [[_upgrades select _i, _prices select _i, _roleName], "Magazines", _config_type]];
				...
				case "CfgVehicles": {
					if (getNumber(configFile >> _config_type >> _item >> 'isbackpack') == 1) then { //--- Make sure that this is a backpack
						missionNamespace setVariable [format["wfbe_%1", _item], [[_upgrades select _i, _prices select _i, _roleName], "Backpack", _config_type]];
				...
				case "CfgWeapons": {
					...
					_item_type = switch (getNumber(configFile >> _config_type >> _item >> "type")) do {
						case wfbe_TYPE_RIFLE: {"Rifle"};
						case wfbe_TYPE_PISTOL: {"Pistol"};
						case wfbe_TYPE_LAUNCHER: {"Launcher"};
						case wfbe_TYPE_RIFLE2H: {"Rifle 2H"};
						case wfbe_TYPE_EQUIPMENT: {"Equipment"};
						case wfbe_TYPE_ITEM: {"Item"};
						default {"nil"};
					};
					...
					if (_item_type == "Item") then { //--- Items are handled differently
						_item_subtype = switch (getNumber(configFile >> _config_type >> _item >> 'ItemInfo' >> 'type')) do {
							case wfbe_SUBTYPE_ITEM: {"BaseItem"};
							case wfbe_SUBTYPE_ACC_MUZZLE: {"Acc-Muzzle"};
							case wfbe_SUBTYPE_ACC_OPTIC: {"Acc-Optics"};
							case wfbe_SUBTYPE_ACC_SIDE: {"Acc-Side"};
							case wfbe_SUBTYPE_ACC_BIPOD: {"Acc-Bipod"};
							case wfbe_SUBTYPE_HEADGEAR: {"Headgear"};
							case wfbe_SUBTYPE_VEST: {"Vest"};
							case wfbe_SUBTYPE_UNIFORM: {"Uniform"};
							default {""};
						};
					...
					missionNamespace setVariable [format["wfbe_%1", _item], [[_upgrades select _i, _prices select _i, _roleName], _item_type, _config_type]];
					_cpt = _cpt + 1;
				};
			};
		};
...
} forEach [[format["wfbe_gear_list_primary_%1", _roleName], _gear_primary],
            [format["wfbe_gear_list_secondary_%1", _roleName], _gear_secondary],
            [format["wfbe_gear_list_pistol_%1", _roleName], _gear_pistol],
            [format["wfbe_gear_list_magazines_%1",_roleName], _gear_magazines],
            [format["wfbe_gear_list_accessories_%1", _roleName], _gear_accessories],
            [format["wfbe_gear_list_misc_%1", _roleName], _gear_misc],
            [format["wfbe_gear_list_special_%1",_roleName], _gear_special],
            [format["wfbe_gear_list_uniforms_%1",_roleName], _gear_uniforms],
            [format["wfbe_gear_list_vests_%1",_roleName], _gear_vests],
            [format["wfbe_gear_list_backpacks_%1", _roleName], _gear_backpacks],
            [format["wfbe_gear_list_headgear_%1", _roleName], _gear_headgear],
            [format["wfbe_gear_list_glasses_%1", _roleName], _gear_glasses],
            [format["wfbe_gear_list_explosives_%1", _roleName], _gear_explosives]];