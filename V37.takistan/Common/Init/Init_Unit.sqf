...
if (_unit_kind in (missionNamespace getVariable "WFBE_REPAIRTRUCKS")) then { //--- Repair Trucks.
	//--- Build action.
	_unit addAction [localize 'STR_WF_BuildMenu_Repair','Client\Action\Action_BuildRepair.sqf', [], 1000, false, true, '', Format['side player == side _target && alive _target && player distance _target <= %1', missionNamespace getVariable 'WFBE_C_UNITS_REPAIR_TRUCK_RANGE']];
	_unit addAction [localize 'STR_WF_Repair_Camp','Client\Action\Action_RepairCamp.sqf', [], 97, false, true, '', '[_unit] call WFBE_CL_FNC_Client_GetNearestCamp'];
...
if (_unit isKindOf "Tank" || _unit isKindOf "Wheeled_APC_F" || typeOf _unit in (missionNamespace getVariable Format["WFBE_%1REPAIRTRUCKS", _side])) then { //--- Tanks.
	//--- Valhalla Low gear.
...
if (_unit isKindOf "Air") then { //--- Air units.
	...
	//--- AAR Tracking.
	if (WFBE_Client_SideJoined != _side) then { //--- Track the unit via AAR System, skip if the unit side is the same as the player one.
		[_unit, _side] call WFBE_CO_FNC_TRACK_AIR_TARGETS;
	};
...
if (_isMan) then { //--- Man.
	...
	if (group _unit == group player) then {
		_color = "ColorOrange";
		_txt = (_unit) Call WFBE_CO_FNC_GetAIDigit;
	};
	...
	if (_unit_kind in (missionNamespace getVariable Format['WFBE_%1REPAIRTRUCKS',str _side])) then {_type = "mil_dot";};//--- Repair.
	...
	_params = [_type,_color,_size,_txt,_markerName,_unit,1,true,"waypoint",_color,false,_side,[2,2]];
        if (_unit == ((_side) Call WFBE_CO_FNC_GetSideHQ)) then {_color = "ColorOrange";_params = ['b_hq',_color,[1,1],'HQ','HQUndeployed',_unit,0.2,false,'','',false,_side];};//--- HQ.
};

if !(_isMan) then { //--- Vehicle Specific.
	if ((missionNamespace getVariable "WFBE_C_GAMEPLAY_MISSILES_RANGE") != 0) then { //--- Max missile range.
		_unit addEventHandler ['incomingMissile', {_this Spawn WFBE_CO_FNC_HandleIncomingMissile}]; //--- Handle incoming missiles.
	};
};

_params Spawn WFBE_CO_FNC_MarkerUpdate;
