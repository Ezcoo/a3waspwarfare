_hq = (side player) Call WFBE_CO_FNC_GetSideHQ;
sleep 5;
...
_logic = (WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideLogic;
_sleepTime = 3;

if(!isNil {_logic getVariable "wfbe_votetime"}) then {
	_sleepTime = _sleepTime + (_logic getVariable "wfbe_votetime");
};
