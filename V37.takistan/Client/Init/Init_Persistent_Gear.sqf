...
{
	_roleTemplates = (missionNamespace getVariable format["wfbe_gear_list_templates_%1_%2", WFBE_Client_SideJoined, _x]);
...
_side_gear = [];
{
	_side_gear = _side_gear + ((missionNamespace getVariable _x) call WFBE_CO_FNC_ArrayToLower);
} forEach ["WFBE_gear_list_primary","WFBE_gear_list_secondary","WFBE_gear_list_pistol","WFBE_gear_list_magazines","WFBE_gear_list_accessories","WFBE_gear_list_misc","WFBE_gear_list_special","WFBE_gear_list_uniforms","WFBE_gear_list_vests","WFBE_gear_list_backpacks","WFBE_gear_list_headgear","WFBE_gear_list_glasses","WFBE_gear_list_explosives"];
					...
					if (typeName _gear_sub == "ARRAY") then {
						{
							...
							if (typeName _x == "ARRAY") then {
								if (count _x == 3) then { //--- Make sure that we have the weapon/accessories/current magazine
									...
									_accessories = (_x select 1) call WFBE_CO_FNC_ArrayToLower;
									_magazine = (_x select 2) call WFBE_CO_FNC_ArrayToLower;
									...
									if (typeName _weapon == "STRING" && typeName _accessories == "ARRAY" && typeName _magazine == "ARRAY") then { //--- The data format is valid
										{
											if (typeName _x == "STRING") then { //--- The accessory is a string
												if (_x != "") then { //--- Empty accessories are skipped
													...
													if (getNumber(configFile >> "CfgWeapons" >> _x >> "type") != WFBE_TYPE_ITEM) exitWith {_flag_load = false}; //--- The accessory is not a valid base class!
													if !(getNumber(configFile >> "CfgWeapons" >> _x >> "ItemInfo" >> "type") in [WFBE_SUBTYPE_ACC_MUZZLE,WFBE_SUBTYPE_ACC_OPTIC,WFBE_SUBTYPE_ACC_SIDE]) exitWith {_flag_load = false}; //--- The accessory is not a valid base class (we don't care bout the order)!
												};
											};
									...
									if (typeName _x == "ARRAY") then {
										if (count _x == 2) then { //--- Each array need to have the container and the items
											if (typeName (_x select 0) == "STRING" && typeName (_x select 1) == "ARRAY") then { //--- The container is a string, the items are an array
												...
												_items = (_x select 1) call WFBE_CO_FNC_ArrayToLower;
					...
					if (_flag_load) then {
						...
						if (_flag_load) then {
								...
								if (count _gear_sub == 2) then {
									...
									if (typeName (_gear_sub select 1) == "ARRAY") then {
										if (count(_gear_sub select 1) == 5) then {
											{
												if (_x != "") then {
													if (getNumber(configFile >> "CfgWeapons" >> _x >> "ItemInfo" >> "type") != WFBE_SUBTYPE_ITEM) 
												};
							...
							if (_flag_load) then {
								...
								_flat = (_x select 3) call WFBE_CO_FNC_ConvertGearToFlat;
								_cost = 0;
								{
...
if (count _list > 0) then { //--- If we have more than one template then we overwrite the existing one
	missionNamespace setVariable ["WFBE_gear_list_templates", _list];
};