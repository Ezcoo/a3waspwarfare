...
missionNamespace setVariable ["WFBE_C_STRUCTURES_PLACEMENT_METHOD",{
    ...
    _area = [_preview,((WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideLogic) getVariable "wfbe_basearea"] Call WFBE_CO_FNC_GetClosestEntity2;
    ...
    if ({_preview isKindOf _x} count _affected != 0) then {
        ...
        _sort = [_preview,_strs] Call WFBE_CO_FNC_SortByDistance;
    ...
    if (_itemcategory == 2) then {
        ...
        if(count (nearestObjects [_preview,missionNamespace getVariable (Format["WFBE_%1DEFENSENAMES",WFBE_Client_SideJoined]),((((boundingbox _preview) select 1) select 0) max (((boundingbox _preview) select 1) select 1)) max 2] - [_preview]) > 0) then {_color = _colorRed} else{_color = _colorGreen};
        _entities = (position _preview) nearEntities [['Man','Car','Motorcycle','Tank','Air','Ship'],12];
        if ((count _entities > 0) && {side _x != WFBE_Client_SideJoined} count _entities !=0) then {_color = _colorRed};
        ...
        _sorted = [_preview,_factories] Call WFBE_CO_FNC_SortByDistance;
    ...
    if (typeOf _preview == "Sign_Danger" && !isNull ([_preview,((WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideLogic) getVariable "wfbe_basearea"] Call WFBE_CO_FNC_GetClosestEntity2)) then {
        _color = _colorRed;
    ...
    if ((typeOf _preview) iskindOf "Warfare_HQ_base_unfolded") then {
        ...
        _town = [_preview] Call WFBE_CO_FNC_GetClosestLocation;
        _townside =  (_town getVariable "sideID") Call WFBE_CO_FNC_GetSideFromID;
        ...
        if!(isNil "_townside")then{
            if ((_preview distance _town < 600 && _townside != WFBE_Client_SideJoined) || !isNull _eArea) then {
                _color = _colorRed;
...
if (_extra == "") then {
	_coin setVariable ["BIS_COIN_funds",[(WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideSupply, Call WFBE_CL_FNC_GetPlayerFunds]];
...
_structures = [(missionNamespace getVariable Format["WFBE_%1STRUCTURENAMES",WFBE_Client_SideJoinedText]) select 0];
_structureCosts = [(missionNamespace getVariable Format["WFBE_%1STRUCTURECOSTS",WFBE_Client_SideJoinedText]) select 0];
_structureDescriptions = [(missionNamespace getVariable Format["WFBE_%1STRUCTUREDESCRIPTIONS",WFBE_Client_SideJoinedText]) select 0];
...
if (_extra == "REPAIR") then {_updateDefenses = true; _emptyStructures = true;_coin setVariable ["BIS_COIN_funds",Call WFBE_CL_FNC_GetPlayerFunds]};
...
if (_updateStructures) then {
	...
	_structureCosts = missionNamespace getVariable Format["WFBE_%1STRUCTURECOSTS",WFBE_Client_SideJoinedText];
	_structureDescriptions = missionNamespace getVariable Format["WFBE_%1STRUCTUREDESCRIPTIONS",WFBE_Client_SideJoinedText];
};
