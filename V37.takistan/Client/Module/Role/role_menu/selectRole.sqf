...
WSW_IsRoleListUpdated = false;

_roleDetails = [_roleName, WFBE_Client_SideJoined] call WSW_fnc_getRoleDetails;
...
if ((_roleDetails select 0) in WSW_gbl_boughtRoles) then {
    ctrlEnable[2804, false];
...
[[[WFBE_Client_SideJoined, player, _roleName], "Server\Module\Role\getRoleList.sqf"], "BIS_fnc_execVM", true, true] call BIS_fnc_MP;