...
_vehi = [group player,false] Call WFBE_CO_FNC_GetTeamVehicles;
_alives = (units group player) Call WFBE_CO_FNC_GetLiveUnits;
...
//--- Service Point.
_csp = objNull;
_sp = [WFBE_Client_SideJoined, missionNamespace getVariable Format ["WFBE_%1SERVICEPOINTTYPE",WFBE_Client_SideJoinedText],_buildings] Call WFBE_CO_FNC_GetFactories;
if (count _sp > 0) then {
...
_effective = [];
_nearSupport = [];
_spType = missionNamespace getVariable Format ["%1SP",WFBE_Client_SideJoinedText];
_i = 0;
{
	_closestSP = objNull;
	_add = false;
	...
	if (_add) then {
		...
		_desc = [typeOf _x, 'displayName'] Call WFBE_CO_FNC_GetConfigInfo;
		_finalNumber = (_x) Call WFBE_CO_FNC_GetAIDigit;
		_isInVehicle = "";
		if (_x != vehicle _x) then {
			_descVehi = [typeOf (vehicle _x), 'displayName'] Call WFBE_CO_FNC_GetConfigInfo;
	...
	if (_curSel != -1) then {
		...
		if (_veh isKindOf "Man") then {
			...
			if (_veh != _lastVeh || getDammage _veh != _lastDmg) then {
				...
				if !(isNil '_get') then {
					_repairPrice = round(_lastDmg*((_get select QUERYUNITPRICE)/(missionNamespace getVariable "WFBE_C_UNITS_SUPPORT_REPAIR_PRICE")));
				} else {
					_repairPrice = 500;
				};
			};
		...
		if (WF_MenuAction == 1) then {
			WF_MenuAction = -1;
			-_rearmPrice Call WFBE_CL_FNC_ChangePlayerFunds;
			
			//--- Spawn a Rearm thread.
			[_veh,_nearSupport select _curSel,_typeRepair,_spType] Spawn WFBE_CL_FNC_SupportRearm;
		};	
		...
		if (WF_MenuAction == 2) then {
			...
			[_veh,_nearSupport select _curSel,_typeRepair,_spType] Spawn WFBE_CL_FNC_SupportRepair;
		};
		
		//--- Refuel.
		if (WF_MenuAction == 3) then {
			WF_MenuAction = -1;
			-_refuelPrice Call WFBE_CL_FNC_ChangePlayerFunds;
			
			//--- Spawn a Refuel thread.
			[_veh,_nearSupport select _curSel,_typeRepair,_spType] Spawn WFBE_CL_FNC_SupportRefuel;
		};
		
		//--- Heal.
		if (WF_MenuAction == 5) then {
			WF_MenuAction = -1;
			-_healPrice Call WFBE_CL_FNC_ChangePlayerFunds;
			
			//--- Spawn a Healing thread.
			[_veh,_nearSupport select _curSel,_typeRepair,_spType] Spawn WFBE_CL_FNC_SupportHeal;
		};
