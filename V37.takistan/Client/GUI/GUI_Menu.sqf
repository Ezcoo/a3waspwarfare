	...
	if (WF_MenuAction == 4) exitWith {
		...
		if(!isNull(commanderTeam))then{
			if(commanderTeam == group player)then{
				...
				if !(_skip) exitWith {};
				[WFBE_Client_SideJoined, name player] remoteExecCall ["WFBE_SE_PVF_RequestCommanderVote",2];
				voted = true;
	...
	if (WF_MenuAction == 10) then { //added-MrNiceGuy
		...
		if (player != _vehicle) then {
			if (getPos _vehicle select 2 > 3 && !surfaceIsWater (getPos _x)) then {
				[_vehicle, getPos _vehicle, 15] Call WFBE_CO_FNC_PlaceSafe;
			} else {
			...
			if (count _objects > 0) then {
				{
					if (getPos _x select 2 > 3 && !surfaceIsWater (getPos _x)) then {
						[_x, getPos _x, 15] Call WFBE_CO_FNC_PlaceSafe;
					} else {
