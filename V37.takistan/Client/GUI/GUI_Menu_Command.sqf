...
{lbAdd [14012,Format["[%1] %2",_u,name (leader _x)]];_u = _u + 1} forEach WFBE_Client_Teams;
lbSetCurSel [14012,1];
...
_templates = missionNamespace getVariable Format["WFBE_%1AITEAMTEMPLATEDESCRIPTIONS",WFBE_Client_SideJoinedText];
_templates_upgrades = missionNamespace getVariable Format["WFBE_%1AITEAMUPGRADES",WFBE_Client_SideJoinedText];
...
_teams = [];
_team = if (_curSel != 0) then {WFBE_Client_Teams select (_curSel - 1)} else {WFBE_Client_Teams select (_curSel + 1)};
_team_old = _team;

_index = (_team) Call WFBE_CO_FNC_GetTeamType;
if (_index == -1) then {_index = 1};
lbSetCurSel [14010,[14010,_index] Call WFBE_CL_FNC_UIFindLBValue];

_txt = (_team) Call WFBE_CO_FNC_GetTeamMoveMode;
...
//_map ctrlMapAnimAdd [1,.075,getPos (leader(WFBE_Client_Teams select 0))];
...
_structures = _structures + ((WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideStructures);
...

{
	if (typeName _x == "OBJECT") then {
		...
		_type = [typeOf _x, 'displayName'] Call WFBE_CO_FNC_GetConfigInfo;
...
while {alive player && dialog} do {
	if (side player != WFBE_Client_SideJoined) exitWith {activeAnimMarker = false;closeDialog 0};
	...
	_team = if !(_isAll) then {WFBE_Client_Teams select (_curSel - 1)} else {WFBE_Client_Teams select (_curSel + 1)};
	...
	if (_updateTab) then {
		...
		switch (_mode) do {
			...
			case 2: {
				...
				_detailGroup = if (!_isAll) then {(units(WFBE_Client_Teams select (_curSel - 1))) Call WFBE_CO_FNC_GetLiveUnits} else {[]};
				[_detailGroup,14041] Call WFBE_CL_FNC_UIFillListTeamOrders;
	...
	//--- Team Funds.
	ctrlSetText [14028,Format [localize 'STR_WF_COMMAND_Cash',str ((_team) Call WFBE_CO_FNC_GetTeamFunds)]];
	...
	if (WF_MenuAction == 1) then {
		...
		_index = (_team) Call WFBE_CO_FNC_GetTeamType;
		_currentCoord = (_team) Call WFBE_CO_FNC_GetTeamMovePos;
		_currentMission = (_team) Call WFBE_CO_FNC_GetTeamMoveMode;
		if (typeName _currentCoord == "ARRAY") then {
			if (count _currentCoord > 0) then {
				_position = _currentCoord;
				if (_currentMission == "move") then {["TempAnim",_position,"selector_selectedMission",1,"ColorOrange",1,1.2] Spawn WFBE_CL_FNC_MarkerAnim};
				if (_currentMission == "patrol") then {["TempAnim",_position,"selector_selectedMission",1,"ColorYellow",1,1.2,"areaPatrol"] Spawn WFBE_CL_FNC_MarkerAnim};
				if (_currentMission == "defense") then {["TempAnim",_position,"selector_selectedMission",1,"ColorRed",1,1.2] Spawn WFBE_CL_FNC_MarkerAnim};
			};
		};
		if (typeName _currentCoord == "OBJECT") then {
			if (_currentMission == "towns") then {["TempAnim",getPos _currentCoord,"selector_selectedMission",1,"ColorBlue",1,1.2] Spawn WFBE_CL_FNC_MarkerAnim};
		};
	...
	if (WF_MenuAction == 101) then {
		WF_MenuAction = -1;
		if (!_isAll) then {
			[_team,'towns'] Call WFBE_CO_FNC_SetTeamMoveMode;
		} else {
			{[_x,'towns'] Call WFBE_CO_FNC_SetTeamMoveMode} forEach WFBE_Client_Teams;
		};
		activeAnimMarker = false;
	};
	...
	if (mouseButtonUp == 0) then {
		...
		if (WF_MenuAction == 102 || WF_MenuAction == 103 || WF_MenuAction == 104) then {
			...
			[_curSel,_position,_order,_isAll,_teams] Spawn {
				...
				if (!_isAll) then {
					...
					for '_i' from 0 to count _teams-1 do {
						_selected = WFBE_Client_Teams select ((_teams select _i) - 1);
					...
					player kbTell [sideHQ, WFBE_V_HQTopicSide, "OrderSent",["1","",_sideTeam,[if (WFBE_Client_SideJoined == west) then {"blueTeam"} else {"redTeam"}]],["2","","moving to position",["HC_MovingToPosition"]],["3","","over.",["Over1"]],true];
					{
						_selected = WFBE_Client_Teams select (_x - 1);
						[_selected,_position] Call WFBE_CO_FNC_SetTeamMovePos;
						[_selected,_order] Call WFBE_CO_FNC_SetTeamMoveMode;
					} forEach _teams;
				} else {
					...
					{
						[_x,_position] Call WFBE_CO_FNC_SetTeamMovePos;
						[_x,_order] Call WFBE_CO_FNC_SetTeamMoveMode;
			...
			_array Spawn WFBE_CL_FNC_MarkerAnim;
		};
			...
			if (!_isAll) then {
				...
				
				{
					...
					if (alive (leader _selected) && isPlayer(leader _selected)) then {
						[_taskType,_taskTime,_taskTimeLabel,_position] remoteExecCall ["WFBE_CL_FNC_SetTask"];
					};
				} forEach _teams;
			} else {
				player kbTell [sideHQ, WFBE_V_HQTopicSide, "OrderSentAll",["1","","All",["all"]],["2","","moving to position",["HC_MovingToPosition"]],["3","","over.",["Over1"]],true];
				[_taskType,_taskTime,_taskTimeLabel,_position] remoteExecCall ["WFBE_CL_FNC_SetTask",WFBE_Client_SideJoined];
			};
		};
	};
	...
	if (WF_MenuAction == 201) then {
		...
		[_team] Spawn {
			...
			if ((missionNamespace getVariable "WFBE_C_RESPAWN_MOBILE") > 0 || ((missionNamespace getVariable "WFBE_C_RESPAWN_CAMPS_MODE") > 0)) then {//--- Make sure that the unit won't spawn back at a camp/ambu.
				[_team,"forceRespawn"] Call WFBE_CO_FNC_SetTeamRespawn;
				sleep 2;
			};
			_units = _units + ([_team,false] Call WFBE_CO_FNC_GetTeamVehicles);
	...
	if (WF_MenuAction == 302) then {
		...
		if (!_isAll) then {
			{
				_selected = WFBE_Client_Teams select (_x - 1);
				[_selected,_curType] Call WFBE_CO_FNC_SetTeamType;
			} forEach _teams;
		} else {
			{[_x,_curType] Call WFBE_CO_FNC_SetTeamType} forEach WFBE_Client_Teams;
		};
	};	
	...
	if (WF_MenuAction == 303) then {
		...
		[_to,_behavior,_combat,_formation,_speed] remoteExecCall ["WFBE_SE_PVF_RequestTeamUpdate",2];
	};	
	...
	if (WF_MenuAction == 304) then {
		...
		if (!_isAll) then {
			{
				_selected = WFBE_Client_Teams select (_x - 1);
				[_selected,(_structures select _curSel)] Call WFBE_CO_FNC_SetTeamRespawn;
			} forEach _teams;
		} else {
			{[_x,(_structures select _curSel)] Call WFBE_CO_FNC_SetTeamRespawn} forEach WFBE_Client_Teams;
		};
	};
	...
	if (_updateRespawn) then {
		...
		_respawn = (_team) Call WFBE_CO_FNC_GetTeamRespawn;
