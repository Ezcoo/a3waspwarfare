...
WFBE_WF_MenuAction = -1;

while {alive player && dialog} do {
	if (WFBE_WF_MenuAction == 1) then {WFBE_WF_MenuAction = -1; if (_player_commander) then {_purchase = true}};
	if (WFBE_WF_MenuAction == 2) then {WFBE_WF_MenuAction = -1;_update_upgrade = true};
	...
	if (_update_upgrade_details) then {
		...
		if (_ui_lnb_sel != -1) then {
			...
			_supply = (WFBE_Client_SideJoined) call WFBE_CO_FNC_GetSideSupply;
			_html = "";
				...
				if (_upgrade_current < (_upgrade_levels select _id)) then {
					...
					if(_funds >= _upgrade_price && _supply >= _upgrade_supply) then {
						...
						if !(_link_needed) then {
							-(_upgrade_price) Call WFBE_CL_FNC_ChangePlayerFunds;
							[WFBE_Client_SideJoined, -(_upgrade_supply)] Call WFBE_CO_FNC_ChangeSideSupply;
							//--- todo check conditions., deduce cash etc
							[WFBE_Client_SideJoined, _id, _upgrade_current, true] remoteExecCall ["WFBE_SE_PVF_RequestUpgrade",2];
							...
							if !(isServer) then {
								...
								[_id, _upgrade_current, _upgrade_time] spawn {
									...
									["upgrade-sync", WFBE_Client_SideJoined, _this select 0, _this select 1] remoteExecCall ["WFBE_SE_PVF_RequestSpecial",2];
								};
							};
	...
	if (WFBE_WF_MenuAction == 1000) exitWith {
		WFBE_WF_MenuAction = -1;
		closeDialog 0;
