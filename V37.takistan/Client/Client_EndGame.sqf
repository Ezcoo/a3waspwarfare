...
//_track = ["EP1_Track13",91]; //---changed-MrNiceGuy
// playMusic _track;
// playMusic "LeadTrack01a_F"; //--There was it--
// playMusic ["Track01_Proteus", 54];
...
_track = [];
{
	if (missionNamespace getVariable Format["WFBE_%1_PRESENT", _x]) then {
		...
		_track = _track + ([_hq, (_x) Call WFBE_CO_FNC_GetSideStructures] Call WFBE_CO_FNC_SortByDistance);
	};
} forEach ([west,east,resistance] - [_side]);

_hq = (_side) Call WFBE_CO_FNC_GetSideHQ;
_blist = [_hq] + _track_hq + ([_hq, (_side) Call WFBE_CO_FNC_GetSideStructures] Call WFBE_CO_FNC_SortByDistance) + _track;
...
_vehi setPos ([getPos _hq,20,30] Call WFBE_CO_FNC_GetRandomPosition);
