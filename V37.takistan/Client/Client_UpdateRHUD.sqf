...
while {true} do {
	...
	if (!isNull (["currentCutDisplay"] call BIS_FNC_GUIget)) then {
		...
		//UPTIME
		_uptime = Call WFBE_CL_FNC_GetTime; //added-MrNiceGuy
		...
		_textControl2 ctrlSetTextColor [0, 0.825294, 0.449803, 1];_textControl2 ctrlSetText Format ["%1 $",Call WFBE_CL_FNC_GetPlayerFunds];
		_textControl3 = (["currentCutDisplay"] call BIS_FNC_GUIget) DisplayCtrl 1353;
		_textControl3 ctrlShow true;
		_textControl3 ctrlSetTextColor [0, 0.825294, 0.449803, 1];_textControl3 ctrlSetText Format ["+ %1 $",WFBE_Client_SideJoined Call WFBE_CL_FNC_GetIncome];
		
		//SUPPLY
		_textControl7 = (["currentCutDisplay"] call BIS_FNC_GUIget) DisplayCtrl 1355;
		_textControl7 ctrlShow true;
		_textControl7 ctrlSetTextColor [1, 0.8831, 0, 1];_textControl7 ctrlSetText Format ["%1",(WFBE_Client_SideJoined) Call WFBE_CO_FNC_GetSideSupply];
		_textControl8 = (["currentCutDisplay"] call BIS_FNC_GUIget) DisplayCtrl 1357;
		_textControl8 ctrlShow true;
		_textControl8 ctrlSetTextColor [1, 0.6831, 0, 1];_textControl8 ctrlSetText Format ["+ %1", WFBE_Client_SideJoined Call WFBE_CO_FNC_GetTotalSupplyValue];
		_textControl9 = (["currentCutDisplay"] call BIS_FNC_GUIget) DisplayCtrl 1359;
		_textControl9 ctrlShow true;
		_textControl9 ctrlSetTextColor [0.1, 0.7, 0.9, 1];_textControl9 ctrlSetText Format ["%1 on %2", WFBE_Client_SideJoined Call WFBE_CO_FNC_GetTownsHeld, _total];
		...
		_textControl10 ctrlSetText Format ["%1 FPS %2",_clientFPS,_level];
