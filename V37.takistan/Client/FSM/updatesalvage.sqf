...
while {!WFBE_GameOver || !(alive _vehicle)} do {
	if(!(isNull (driver _vehicle)) && !WFBE_GameOver && alive _vehicle) then
	{
		...
		_overAllCost = 0;
		{
			...
			if (isNil '_isNeeded') then {
				...
				(Format [localize 'STR_WF_CHAT_Salvaged_Unit',_salvageCost,[typeOf _x,'displayName'] Call WFBE_CO_FNC_GetConfigInfo]) Call WFBE_CL_FNC_GroupChatMessage;
		
				deleteVehicle _x;
			};
		} foreach _wrecks;

		if (_overAllCost > 0) then {(_overAllCost) Call WFBE_CL_FNC_ChangePlayerFunds};
	};
	sleep 5;
};