...

{
	...
	lnbAddRow [_listBox,[str(round(((getDammage _unit)*-100)+100)) + "%",_extra+"("+([typeOf _unit, 'displayName'] Call WFBE_CO_FNC_GetConfigInfo)+") "+name _x]];
	_pic = if (_isVehicle) then {[typeOf _unit, 'picture'] Call WFBE_CO_FNC_GetConfigInfo} else {[typeOf _unit, 'portrait'] Call WFBE_CO_FNC_GetConfigInfo};
