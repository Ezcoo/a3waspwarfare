...
[Format[Localize "STR_WF_CHAT_Town_Captured", _town getVariable "name", _side_label]] Call WFBE_CL_FNC_TitleTextMessage;
...
//--- Taskman
["TownUpdate", _town] Spawn WFBE_CL_FNC_TaskSystem;
...
if (_town_side_value_new == WFBE_Client_SideID) then {
	...
	if !(isNull _closest) then {
		...
		if (_bonus != -1) then {
			(_bonus) Call WFBE_CL_FNC_ChangePlayerFunds;
			Format[Localize "STR_WF_CHAT_Town_Bounty_Full", _town getVariable "name", _bonus] Call WFBE_CL_FNC_CommandChatMessage;
		};
		
		//--- Update the score necessary.
		if (_score != -1) then {
			[player,score player + _score] remoteExecCall ["WFBE_SE_PVF_RequestChangeScore",2];
		};
	};
	...
	if !(isNull commanderTeam) then {
		if (commanderTeam == group player) then {
			...
			(_bonus) Call WFBE_CL_FNC_ChangePlayerFunds;
			[player,score player + (missionNamespace getVariable "WFBE_C_PLAYERS_COMMANDER_SCORE_CAPTURE")] remoteExecCall ["WFBE_SE_PVF_RequestChangeScore",2];
			Format[Localize "STR_WF_CHAT_Commander_Bounty_Town", _bonus, _town getVariable "name"] Call WFBE_CL_FNC_CommandChatMessage;
		};
	};
	...
	if !(isNull _task) then {
		if (_ptask == _task) then {
			["TownAssignClosest"] Spawn WFBE_CL_FNC_TaskSystem;
		};
	};
};