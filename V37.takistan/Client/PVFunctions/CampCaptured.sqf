	...
	// if ((WFBE_Client_Team getVariable "wfbe_task_order") == "towns") then {
		...
		// if ((WFBE_Client_Team getVariable "wfbe_task_position") == _town) then {
			...
			if (_closest distance _camp < (missionNamespace getVariable "WFBE_C_CAMPS_RANGE")) then {
				...
				[player,score player + (missionNamespace getVariable 'WFBE_C_PLAYERS_SCORE_CAPTURE_CAMP')] remoteExecCall ["WFBE_SE_PVF_RequestChangeScore",2];
				
				(missionNamespace getVariable "WFBE_C_CAMPS_CAPTURE_BOUNTY") Call WFBE_CL_FNC_ChangePlayerFunds;
				["INFORMATION", Format ["Player %1 has captured camp in town: %2).", name player, _town getVariable "name"]] Call WFBE_CO_FNC_LogContent;
			};
		// };
	// };
} else {
