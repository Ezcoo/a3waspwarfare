	...
	if (isClass(configFile >> 'CfgWeapons' >> (_u select _i))) then {
		if (isNil {missionNamespace getVariable Format["%1",_u select _i]}) then {
			...
			switch (getNumber(configFile >> 'CfgWeapons' >> (_u select _i) >> "type")) do {
				...
				case 5: {_belong = 0;_size = 10;_is = "Primary"};
			...
			if (_belong != -1) then { 
				...
				if(Format["%1",_u select _i] != "Binocular")then{
					missionNamespace setVariable [Format["%1",_u select _i], _set];
				}else{
					missionNamespace setVariable [Format["WF_%1",_u select _i], _set];
				};
				
				_cpt = _cpt + 1;
				["INITIALIZATION", Format["Config_Weapons.sqf : [%1] Weapon [%2], Belong to [%3]",_faction,_u select _i,_is]] Call cti_CO_FNC_LogContent;
			} else {
				["WARNING", Format["Config_Weapons.sqf : [%1] Weapon [%2] was not defined since it's gear emplacement is unknown (type)",_faction,_u select _i], 2] Call cti_CO_FNC_LogContent;
			};
		} else {
			_cpt = _cpt + 1;
			["TRIVIAL", Format["Config_Weapons.sqf : [%1] Weapon [%2] has been skipped since it's already defined",_faction,_u select _i]] Call cti_CO_FNC_LogContent;
		};
	} else {
		["ERROR", Format["Config_Weapons.sqf : [%1] Weapon [%2] is not a valid class within <CfgWeapons>",_faction,_u select _i], 3] Call cti_CO_FNC_LogContent;
	};
};

["INFORMATION", Format ["Config_Weapons.sqf : [%1] [%2/%3] Entities were defined.", _faction, _cpt, count _u], 1] Call cti_CO_FNC_LogContent;