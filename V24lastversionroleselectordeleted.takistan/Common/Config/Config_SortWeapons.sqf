...
_sorted = [[],[],[],[]];
_getall = missionNamespace getVariable Format ["cti_%1_All", _side];
...

{
	...
	if !(isNil '_get') then {
		...
		if (_index != -1) then {
			...
			["ERROR", Format["Config_SortWeapons.sqf : [%1] Weapon [%2] does not belong to any known pool.", _side, _x]] Call cti_CO_FNC_LogContent;
		};
	} else {
		["ERROR", Format["Config_SortWeapons.sqf : [%1] Weapon [%2] is not defined inside the Gear files.", _side, _x]] Call cti_CO_FNC_LogContent;
	};
} forEach _w;
...
} forEach [[Format ["cti_%1_Primary", _side], _sorted select 0],[Format ["cti_%1_Pistols", _side], _sorted select 1],[Format ["cti_%1_Secondary", _side], _sorted select 2],[Format ["cti_%1_Equipment", _side], _sorted select 3],[Format ["cti_%1_All", _side], (_sorted select 0) + (_sorted select 1) + (_sorted select 2) + (_sorted select 3) + _m]];