...
cti_REQ_ADDONS = 	[	//["@Bornholm", ["Bornholm"]], 
...
//--- Environment.
	if (isNil "cti_C_OBJECT_MAX_VIEW") then {cti_C_OBJECT_MAX_VIEW = 5000;}; //--- Max object distance.
	if (isNil "cti_C_ENVIRONMENT_MAX_VIEW") then {cti_C_ENVIRONMENT_MAX_VIEW = 5000;}; //--- Max view distance.
	if (isNil "cti_C_ENVIRONMENT_MAX_CLUTTER") then {cti_C_ENVIRONMENT_MAX_CLUTTER = 35;}; //--- Max Terrain grid.
...
//--- Gameplay.
	...
	if (isNil "cti_C_GAMEPLAY_MISSILES_RANGE") then {cti_C_GAMEPLAY_MISSILES_RANGE = 3500;}; //--- missile range limit
...
//--- Players.
	...
	cti_C_PLAYERS_HALO_HEIGHT = 200; //--- Distance above which units are able to perform an HALO jump.
...
//--- Towns.
	...
	if (isNil "cti_C_TOWNS_AI_SPAWN_RANGE") then {cti_C_TOWNS_AI_SPAWN_RANGE = 1000;};
	...
	if (isNil "cti_C_TOWNS_GEAR") then {cti_C_TOWNS_GEAR = 1;}; //--- Buy Gear From (0: None, 1: Camps, 2: Depot, 3: Camps & Depot).
...
//--- Units.
	...
	if (isNil "cti_C_UNITS_CLEAN_TIMEOUT") then {cti_C_UNITS_CLEAN_TIMEOUT = 60;}; //--- Lifespan of a dead body.
	...
	cti_C_STATIC_DEFENCE_FOR_COMPOSITIONS = ['CUP_I_DSHKM_TK_GUE','CUP_I_2b14_82mm_TK_GUE'];
