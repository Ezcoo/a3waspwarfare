...
while {alive player && dialog} do {
	...
	if (_currentSel != _lastSel || _forceReload) then {
        ...
        if!(isNil '_selectedRole')then{
            if(_selectedRole == WSW_UAV_OPERATOR)then{
                _currentFee = _currentFee - (_currentFee * CTI_ADV_UAV_DISCOUNT);
            };
        };
	...
	if (WF_MenuAction == 2) then {
		WF_MenuAction = -1;
		_units = [Group player,false,lbCurSel(17008),CTI_Client_SideJoinedText] Call CTI_CO_FNC_GetTeamArtillery;
		if (Count _units > 0) then {
			fireMissionTime = time;
			[GetMarkerPos "artilleryMarker",lbCurSel(17008)] Spawn CTI_CL_FNC_RequestFireMission;
		} else {
	...
	if (WF_MenuAction == 200 || _startLoad) then {
		...
		_trackingArray = [group player,true,lbCurSel(17008),CTI_Client_SideJoined] Call CTI_CO_FNC_GetTeamArtillery;
