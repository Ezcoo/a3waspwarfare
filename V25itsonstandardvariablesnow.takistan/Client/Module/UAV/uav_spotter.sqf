OperatorUav_delay = 5;  			//Check for new targets every x sec				//co ile sek. sprawdzać nowości
OperatorUav_refreshrate = 1; 		//Refresh markers every x sec. Should be divisible by OperatorUav_delay	//co ile sek. odświeżać markery. Powinno byc podzielne przez OperatorUav_delay
...
OperatorUav_maxRange = 1500;		//Maximal range. Not higher than 6000.
OperatorUav_range = 1;
...
OperatorUavGetInSight = {
	...
	} foreach (_uav nearObjects ["AllVehicles", OperatorUav_range]);
	_Uav setVariable ["UavInSight",_UavInSight];
	{
	If ((time -((_x getVariable "UavLastSeen")select 0)) <  OperatorUav_delay) then{
	[_x,_Uav] spawn OperatorUavMarker}} foreach _UavInSight //Deprecated since PvP update by OperatorUavPlayer
};
    ...
    If (side (_this select 1) == playerSide) then {
        ...
        If (!(_target getVariable ["UavHasMarker",false])) then {
            ...
            While {(_target in _UavInSight) AND (time - _timestamp) < OperatorUav_disappear} do{
                While {(_target in _UavInSight) AND ((time - _timestamp) < OperatorUav_delay)} do {
...
While {Alive _uav && fuel _uav > 0.05} do{
    _script = _uav spawn OperatorUavGetInSight;
