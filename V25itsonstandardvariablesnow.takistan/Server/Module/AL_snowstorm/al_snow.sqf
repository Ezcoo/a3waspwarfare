...
null = [] execVM "Server\Module\AL_snowstorm\alias_hunt.sqf";
//null = [] execvm "Server\Module\AL_snowstorm\umblator.sqf";
...
if (snow_burst_server_side) then {
	...
	while {al_snowstorm_om} do {
		...
		[[_dir,dist,_vect],"Server\Module\AL_snowstorm\rotocol.sqf"] remoteExec ["execVM"];
		
		_dir_1 = ["x","y"] call BIS_fnc_selectRandom;
		_vect_1 = [0.01,-0.01] call BIS_fnc_selectRandom;
		[[_dir_1,dist,_vect_1],"Server\Module\AL_snowstorm\rotocol.sqf"] remoteExec ["execVM"];
...
if (snow_burst_client_side) then {
	...
	if (terrain_type_vanilla) then {
		...
		[[],"Server\Module\AL_snowstorm\rotocol_client.sqf"] remoteExec ["execVM"];
...
[[],"Server\Module\AL_snowstorm\alias_snowstorm_effect.sqf"] remoteExec ["execVM",0,true];

/*if (breath_vapors_units) then 
{
	{
	[[_x],"Server\Module\AL_snowstorm\snow_breath.sqf"] remoteExec ["execVM",0,true];
	sleep 0.5
	} forEach allUnits;
};*/
