...
//--- Weather
if (CTI_C_ENVIRONMENT_WEATHER_SAND == -1) then {CTI_C_ENVIRONMENT_WEATHER_SAND = selectRandom [0,1,2,3,4];};
	
if (CTI_C_ENVIRONMENT_WEATHER_SAND > 0 || CTI_C_ENVIRONMENT_WEATHER_SNOWSTORM > 0) then {		
	[] spawn {enableEnvironment false;};
	...
	if (hasInterface) then {
		[] spawn {
			...
			if (CTI_C_ENVIRONMENT_WEATHER_SAND == 2) then { 
				_MKY_arSandEFX = [[0.30,0.015,200],"",false,false,true,true,true,2];
			};
			if (CTI_C_ENVIRONMENT_WEATHER_SAND == 3) then { 
				_MKY_arSandEFX = [[0.50,0.010,300],"",false,false,true,true,true,3];
			};
			if (CTI_C_ENVIRONMENT_WEATHER_SAND == 4) then { 
				_MKY_arSandEFX = [[0.75,0.005,400],"",false,false,true,true,true,4];
			};*/			
			
			if(CTI_C_ENVIRONMENT_WEATHER_SNOWSTORM > 0) then { 
